...
time discovery is loaded from the org-graph loader).  Wrapped in
=ignore-errors= so a missing/unbuilt DB at load time never aborts module
loading; the actual behaviour is unit-tested against
=org-graph/seed-org-id-locations= directly.
