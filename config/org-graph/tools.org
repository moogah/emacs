...
API matches relations as SYMBOLS, and every STORED relation passed
through =org-graph-extractor--normalize-rel= on the way in
(=register/vocabulary/relation-types=), so an agent-supplied string must
take the same path — a bare =intern= of ="FOLLOWS_UP"= or ="follows up"=
would silently query a symbol the DB never contains.  =connected=
