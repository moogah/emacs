...
(=org-graph-extractor--tag->rel=, per architecture.md).  When it is
defined (it loads before this module in the loader sequence), delegate
to it so the registry can never fork one authored relation into two
symbols; the local fallback implements the identical rule so this module
stays standalone-loadable (e.g. in a unit spec).
