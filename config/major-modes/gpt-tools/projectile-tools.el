...
(gptel-make-tool
 ...
 :function (lambda (directory search-term &optional file-pattern)
             (let ((project-root (projectile-project-root directory)))
               (if (not project-root)
                   (format "No project found at %s. Use list_known_projects to discover available projects." directory)
                 (condition-case err
 ...
 :description "Find all files containing a specific string or pattern.
                   ...
                   (if (zerop total-count)
                     ...
                     (let ((result (format "Found %d test file%s in project %s:\n\n%s%s"
                                           ...
                                           (if (> total-count file-limit)
                                             ...
                                             ""))))
                       (gptel-projectile--result-limit result)))))))
 :description "Get all test files in the project.
...
(gptel-make-tool
 :name "find_related_test"
 :function (lambda (directory file-path)
             (let ((project-root (projectile-project-root directory)))
               (if (not project-root)
                   (format "No project found at %s. Use list_known_projects to discover available projects." directory)
                   ...
                   (condition-case err
                       (let ((test-files (projectile--related-files absolute-path :test)))
                         (if (null test-files)
                           ...
                           (format "Test file%s for %s:\n\n%s"
                                   (if (> (length test-files) 1) "s" "")
                                   relative-path
                                   (mapconcat (lambda (f) (file-relative-name f project-root)) test-files "\n"))))
                     (error (format "Error finding test for %s: %s" relative-path (error-message-string err))))))))
 :description "Find the test file(s) for a specific implementation file.
...
(gptel-make-tool
 ...
 :function (lambda (directory file-path &optional relationship-type)
             (let ((project-root (projectile-project-root directory)))
               (if (not project-root)
                   (format "No project found at %s. Use list_known_projects to discover available projects." directory)
                 (let* ((absolute-path (if (file-name-absolute-p file-path)
                   ...
                   (condition-case err
                       (if relationship-type
                           ...
                           (let* ((rel-keyword (intern (concat ":" relationship-type)))
                             ...
                             (if (null related)
                                 (format "No %s files found for %s" relationship-type relative-path)
                               (format "%s file%s for %s:\n\n%s"
                         ...
                         (let* ((kinds (projectile--related-files-kinds absolute-path))
                           ...
                           (dolist (kind kinds)
                             (let ((files (projectile--related-files absolute-path kind)))
                               (when files
                                 (push (format "%s:\n  %s"
                                              (capitalize (substring (symbol-name kind) 1))
                                              (mapconcat (lambda (f) (file-relative-name f project-root)) files "\n  "))
                                       results))))
                           (if (null results)
                               (format "No related files found for %s\n\nThis file may be standalone or relationships not configured for this project type." relative-path)
                             (format "Related files for %s:\n\n%s" relative-path (mapconcat #'identity (reverse results) "\n\n")))))
                     (error (format "Error finding related files for %s: %s" relative-path (error-message-string err))))))))
 :description "Find all semantically related files for a given file.
