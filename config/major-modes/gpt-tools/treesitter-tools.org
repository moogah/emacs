#+title: GPTEL Tree-sitter Tools
#+author: Jeff Farr
#+property: header-args:emacs-lisp :tangle treesitter-tools.el
#+auto_tangle: t

* Introduction
Tools for structural code exploration using Emacs's built-in tree-sitter support.

These tools enable LLM agents to navigate code using Abstract Syntax Tree (AST) parsing -
understanding code structure, extracting semantic elements (functions, classes, imports),
and querying code with precision far beyond text-based search.

Tree-sitter provides:
- AST-based code analysis (structural understanding)
- Language-agnostic semantic extraction
- Query-based pattern matching
- Accurate symbol and scope identification
- No external dependencies (built into Emacs 30+)

These tools complement other tool categories:
- *ggtags*: Cross-file navigation (definitions, references, symbol search)
- *projectile*: File discovery and project structure
- *grep*: Text content and comment search
- *Read tool*: Full file content viewing

Tree-sitter excels at understanding code structure within individual files.

* Basic Configuration
#+begin_src emacs-lisp :tangle treesitter-tools.el
...
** Result Limiting
Prevent context overflow by limiting result size to 40,000 characters (matching other tool files).
...
** Language Detection
Detect programming language from file path and get appropriate parser.
...
** Node Type Mappings
Language-specific node type mappings for semantic extraction (functions, classes, imports).
...
Format nodes and results for LLM consumption.
...
Extract function signatures in language-appropriate format.
...
* Language & Parser Management Tools

** list_treesitter_languages
List available tree-sitter language grammars and their installation status.
...
Verify tree-sitter parser availability for a specific file.
...
** get_node_info
Get detailed information about a node at a position.
...
** find_nodes_in_range
Find nodes within a specific line or position range.
...
** get_scope_structure
Understand nesting depth and scope hierarchy for analysis or refactoring.
