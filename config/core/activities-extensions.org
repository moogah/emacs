...
*** Selected Projects List Generator

Generates list showing only selected projects with their action indicators.
Click a project to configure its git action.
...
Simple status display with usage instructions.
...
Three-column layout:
1. Project Selection - all available projects, click to toggle selection
2. Selected Projects - shows selected projects with action indicators [W/B/·]
3. Git Action - appears when clicking a selected project, lets you change its action

New projects default to worktree action. Click a project in Selected Projects column
to configure its git action (worktree, branch, or none).
