...
Disabled on the =add-workspaces-package= branch while the new
=workspaces= package is being tested.  =persp-mode= scopes buffers
per perspective and silently swaps the displayed buffer when its
notion of the current perspective differs from ours, which masks
window-state-capture/restore bugs in =workspaces=.  Re-enable when
the workspaces cutover decision is made.
...
Disabled on the =add-workspaces-package= branch.  =activities-mode=
runs a 5-second idle timer that calls =window-state-get= on the
same frame =workspaces= is managing, and =activities-tabs-mode=
hooks =window-configuration-change-hook= and stamps tab parameters,
both of which add noise to interactive workspace-save / restore
testing.  The package itself stays installed so its functions are
still available via =M-x=; only the auto-activated modes are off.
