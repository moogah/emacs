#+title: Scope-Config Contract Tests — Verifying the Configuration Specification
#+author: Jeff Farr
#+property: header-args:emacs-lisp :tangle contract-scope-config-spec.el
#+auto_tangle: y

* Purpose

These tests validate that the scope-config contract /validators/ correctly accept
valid configuration data and reject invalid or stale data. They are testing the
contracts themselves, not the YAML parser or schema loader.

** The Boundary Under Contract

#+begin_example
┌─────────────────────┐         ┌─────────────────────┐
│   scope.yml          │  ────►  │   scope-validator    │
│   (YAML config)      │  data   │   (CONSUMER)         │
│                      │  flows  │                      │
│ jf/gptel-scope--     │         │ validate-command-    │
│   load-schema        │         │   semantics          │
│                      │         │                      │
└─────────────────────┘         └─────────────────────┘
           │                              │
           │         contracts            │
...
- *Producer:* =scope.yml= via =jf/gptel-scope--load-schema= — YAML loading + defaults merge
- *Consumer:* =config/gptel/scope/= — 7-stage validation pipeline
- *Contract:* =config/test/contracts/contract-scope-config.el= — THIS FILE TESTS THIS

** What These Tests Catch

- *Accept tests:* Valid config passes validation (the contract is not too strict)
- *Reject tests:* Invalid or stale config fails validation (the contract is not too loose)

Key reject cases:
- =:categories= in bash-tools (removed during migration)
- =:true= / =:false= instead of =t= / =nil= (YAML boolean keyword bug)
- Missing required path keys
- Auth-detection values outside the allowed set
...
* Paths Contract Tests

The paths section is the most critical — it defines which file system operations
are allowed. All 5 keys must be present after schema loading, each with a list
of string glob patterns (possibly empty).
...
The cloud section controls how cloud authentication commands are handled.
The =:auth-detection= field must be one of three specific string values.
...
The security section controls parse completeness enforcement. The YAML boolean
keyword bug is a particularly important reject case — YAML parsers may emit
=:true= as a keyword symbol instead of Emacs =t=.
...
* Bash Tools Contract Tests

The bash-tools section is optional. The key migration validation is that
=:categories= must NOT be present — it was removed during the operation-first
migration.
...
* Top-Level Config Contract Tests

The top-level validator delegates to sub-validators. These tests verify the
delegation works correctly and that the required/optional structure is enforced.
...
scope-config-specific contracts.
