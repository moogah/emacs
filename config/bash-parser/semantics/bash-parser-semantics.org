...
We extract the search directory (:read-directory), the pattern (:match-pattern), and operations from -exec blocks by recursively parsing the exec command.
...
Validate the semantics database to catch configuration errors at load time.
