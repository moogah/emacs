;;; token-claiming-spec.el --- Unit tests for jf/bash--claim-tokens-for-results -*- lexical-binding: t; -*-

;;; Commentary:

;; Tests that jf/bash--claim-tokens-for-results correctly claims tokens
;; for all operation source types without throwing errors.
;;
;; The function takes (all-operations entries parsed-command) and claims
;; tokens by matching operation files against token values, plus always
;; claiming redirection, operator, and command-name tokens.
...
(describe "jf/bash--claim-tokens-for-results"
  ...
  (describe "does not throw for any operation source type"
    ...
    (it "handles :source :positional-arg without error"
      (let ((operations '((:file "file.txt"
                           ...
                           :command "cat")))
            (entries (list (list :command (list :command-name "cat"))))
            (parsed-command (list :command-name "cat"
                                 :tokens '((:id 0 :type :command-name :value "cat")
                                           (:id 1 :type :positional-arg :value "file.txt"))
                                 :type :simple)))
        (expect (jf/bash--claim-tokens-for-results operations entries parsed-command)
                :not :to-throw)))
    ...
    (it "handles :source :exec-block without error"
      (let ((operations '((:file "script.sh"
                           ...
                           :source :exec-block
                           :command "find")))
            (entries (list (list :command (list :command-name "find"))))
    ...
    (it "handles :source :flag-arg without error"
      (let ((operations '((:file "output.txt"
                           ...
                           :source :flag-arg
                           :command "sed")))
            (entries (list (list :command (list :command-name "sed"))))
    ...
    (it "handles :source :redirection without error"
      (let ((operations '((:file "output.txt"
                           :operation :write
                           :source :redirection)))
            (entries (list (list :command (list :command-name "echo"))))
      ...
      (let ((operations '((:file "output.txt"
            ...
            (parsed-command (list :command-name "echo"
                                 :tokens '((:id 0 :type :command-name :value "echo")
                                           ...
                                           (:id 3 :type :positional-arg :value "output.txt"))
                                 :type :simple)))
        (let ((claimed (jf/bash--claim-tokens-for-results operations entries parsed-command)))
