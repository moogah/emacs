...
Claims tokens by matching against the top-level :tokens list:
  - All :redirection, :operator, :separator, :pipe, :flag type tokens
    (structural, parse-understood, carry no filesystem semantics)
  - :command-name tokens matching processed command names
  - Tokens whose :value matches an operation :file path
  - For commands with a registered handler: :positional-arg tokens
    matching the command's args, even when the handler emitted no
    operations (a no-op handler registration is enough to mark the
    command as parse-understood)
    ...
    (when tokens
      (dolist (token tokens)
        (let ((token-type (plist-get token :type))
          ...
          (when token-id
            (cond
             ;; Always claim structural tokens — parse-understood,
             ;; no filesystem semantics.
             ((memq token-type
                    '(:redirection :operator :separator :pipe :flag))
             ...
             ((and token-value
              ...
              (push token-id claimed-ids)))))))
    ;; Claim positional-arg tokens for simple commands that have a
    ;; registered handler, regardless of whether the handler emitted
    ;; operations. Covers pure stdout/builtin commands (echo, printf,
    ;; true, false): a no-op handler registration is enough to mark
    ;; the command as parse-understood.
    (when (and tokens (boundp 'jf/bash-command-handlers))
      (let ((commands (or (plist-get parsed-command :all-commands)
                          (list parsed-command))))
        (dolist (cmd commands)
          (let ((cmd-name (plist-get cmd :command-name)))
            (when (and cmd-name
                       (gethash cmd-name jf/bash-command-handlers))
              ;; Claim each positional-arg token matching this command's
              ;; args, first-unclaimed-wins so repeated values across
              ;; commands don't collide.
              (dolist (pos-arg (plist-get cmd :positional-args))
                (catch 'found
                  (dolist (token tokens)
                    (when (and (null (cl-find (plist-get token :id) claimed-ids))
                               (eq (plist-get token :type) :positional-arg)
                               (string= (plist-get token :value) pos-arg))
  ...
  (let ((resolved nil))
    (dolist (op operations)
      (let* ((file (plist-get op :file))
             ...
             (has-pattern nil)
             (unresolved-vars nil))
        ;; Skip operations where the entire file path is a raw command substitution
        ;; e.g., $(find . -name '*.txt') — pattern-flow provides resolved versions
        ;; Keep paths like $(pwd)/file.txt that have content after the closing paren
