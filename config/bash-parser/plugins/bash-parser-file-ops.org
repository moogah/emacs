...
All valid operation types recognized by the file operations extractor.
...
** Helper: Validate Variable Context

Helper function to validate variable context structure.
...
Internal implementation that does the actual extraction work.
This is called by both the public API wrapper and the filesystem plugin.
...
** Filesystem Extraction (Delegates to Orchestrator)

Extract filesystem operations via the two-layer orchestrator.
This function provides a flat-list interface to =jf/bash-extract-semantics=.
...
Validate that operation plists have correct structure and valid operation types.
...
*** Semantics Database Validation

Validate semantics database entries to catch malformed operation specs early.
...
Resolve index specifications to list of target indices.
...
**** Operation Building Helper

Build file operation plist from resolved file path and metadata.
...
*** Helper: Dynamic Path Replacement

When a redirect destination contains command substitutions or unresolvable
variables, replace the unknowable parts with =*= (glob wildcard) while
preserving known structure (directory prefixes, file extensions). The output
is a glob pattern in the same language as the scope policy files, so the
scope validator can match extracted paths against policy globs directly.
