...
variables, replace the unknowable parts with ={dynamic}= while preserving
known structure (directory prefixes, file extensions). This gives the scope
validator maximum information for policy matching.
