#+title: Bash Parser Cloud Auth
#+property: header-args:emacs-lisp :tangle bash-parser-cloud-auth.el
#+auto_tangle: y

* Overview

This module provides a plugin for extracting cloud authentication context from AWS, GCP, and Azure CLI commands.

The plugin identifies authentication-related flags and arguments for:
- AWS CLI (--profile, --region)
- AWS Vault (exec, login with profile/account names)
- Google Cloud (--project, --zone, --region)
- Azure CLI (--subscription, --location)

* Cloud Auth Patterns Database

** Pattern Definitions

Define authentication patterns for major cloud providers.
...
** Helper: Find Pattern for Command
...
** Helper: Extract Account from Position
