...
(defun jf/bash--dynamicize-path (file-path)
  "Replace unresolvable dynamic parts in FILE-PATH with {dynamic}.

Replaces command substitutions $(…) and unresolved $VAR references
with {dynamic}, preserving known directory prefixes and file extensions.

Examples:
  (jf/bash--dynamicize-path \"backup/$(basename \\\"$file\\\")\")
    => \"backup/{dynamic}\"

  (jf/bash--dynamicize-path \"output/$line.txt\")
    => \"output/{dynamic}.txt\"

  (jf/bash--dynamicize-path \"log-$(date +%Y-%m-%d).txt\")
    => \"log-{dynamic}.txt\"

  (jf/bash--dynamicize-path \"backup-$(whoami)-$(date +%s).txt\")
    => \"backup-{dynamic}.txt\"

  (jf/bash--dynamicize-path \"/static/path.txt\")
    => nil  (no dynamic parts detected)

Returns the dynamicized path string, or nil if the path has no dynamic parts."
  (when (and (stringp file-path)
             (or (string-match-p "\\$(" file-path)
                 (string-match-p "\\$[A-Za-z_]" file-path)
                 (string-match-p "`[^`]+`" file-path)))
    ;; Replace command substitutions $(…) — handle nested parens
    (let ((result file-path))
      ;; Replace $(…) including nested parens, iteratively from innermost
      (while (string-match-p "\\$([^)]*)" result)
        (setq result (replace-regexp-in-string
                      "\\$([^)]*)" "{dynamic}" result t)))
      ;; Replace backtick substitutions `…`
      (setq result (replace-regexp-in-string
                    "`[^`]+`" "{dynamic}" result t))
      ;; Replace bare $VAR references (but not already-replaced {dynamic})
      (setq result (replace-regexp-in-string
                    "\\$[A-Za-z_][A-Za-z0-9_]*" "{dynamic}" result t))
      ;; Collapse adjacent {dynamic} segments (e.g., {dynamic}-{dynamic}.txt → {dynamic}.txt)
      (while (string-match-p "{dynamic}[^/]*{dynamic}" result)
        (setq result (replace-regexp-in-string
                      "{dynamic}[^/]*{dynamic}" "{dynamic}" result t)))
      result)))
          ...
          (let ((operation-type (jf/bash--map-redirect-operator-to-operation operator)))
            (when operation-type
              ...
              (let* ((resolved-path (jf/bash--resolve-path-variables destination var-context))
                     ...
                     (unresolved-vars (when (listp resolved-path)
                                       (plist-get resolved-path :unresolved))))
                ;; Detect dynamic paths (unresolvable $(cmd) or $VAR)
                (let ((dynamic-path (jf/bash--dynamicize-path file-path)))
                  ;; Build operation plist
                  (push (append (list :file (or dynamic-path file-path)
                                     ...
                                     :metadata redir)
                               (when dynamic-path
                                 (list :dynamic t :dynamic-filename t))
                               (when unresolved-vars
                                 (list :unresolved t :unresolved-vars unresolved-vars))
                               (when has-heredoc
