...
(defun jf/bash--dynamicize-path (file-path)
  "Replace unresolvable dynamic parts in FILE-PATH with * (glob wildcard).

Replaces command substitutions $(…) and unresolved $VAR references
with *, preserving known directory prefixes and file extensions.
The result is a glob pattern that the scope validator can match
directly against policy paths (e.g., ${project_root}/**).
...
Returns the dynamicized path string, or nil if the path has no dynamic parts."
  (when (and (stringp file-path)
    ...
    (let ((result file-path))
      ...
      (setq result (replace-regexp-in-string
                    "\\$[A-Za-z_][A-Za-z0-9_]*" "*" result t))
      ;; Collapse adjacent * segments (e.g., *-*.txt → *.txt)
      (while (string-match-p "\\*[^/]*\\*" result)
        (setq result (replace-regexp-in-string
                      "\\*[^/]*\\*" "*" result t)))
      result)))
