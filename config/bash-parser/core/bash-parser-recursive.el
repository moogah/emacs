...
Shared by bash-parser-recursive and bash-parser-extensions modules.")
    ...
    (when-let ((substs (plist-get parsed-command :command-substitutions)))
      (dolist (subst substs)
        (when-let ((parsed-subst (plist-get subst :parsed)))
          (if (plist-get parsed-subst :error)
              ;; Log error but continue processing other substitutions
              (lwarn 'bash-parser :error "Substitution parse failed: %s"
                     (plist-get parsed-subst :error))
            ;; Otherwise check success
            (when (plist-get parsed-subst :success)
  ...
  (let* ((loop-ops nil)
    ...
    (when loop-body
      (let* ((loop-body-parsed-raw (jf/bash-parse loop-body))
             ...
             (loop-body-parsed (cond
                               ((plist-get loop-body-parsed-raw :error)
                                (lwarn 'bash-parser :error "Loop body parse failed: %s"
                                       (plist-get loop-body-parsed-raw :error))
                                nil)
                               ((plist-get loop-body-parsed-raw :success)
...
Example:
  ...
  (condition-case err
      (catch 'depth-exceeded
        ...
        (when (eq (plist-get parsed-command :type) :simple)
  ...
  (let ((operations nil))
    ...
    (if (string-match "\\[\\[?\\s-*\\(-[fderswx]\\)\\s-+\\([^]]+\\)\\s-*\\]\\]?" condition-text)
      ...
      (let* ((parsed-test (jf/bash-parse condition-text))
             ...
             (clean-parsed (cond
                           ((plist-get parsed-test :error)
                            (lwarn 'bash-parser :error "Test condition parse failed: %s"
                                   (plist-get parsed-test :error))
                            nil)
                           ((plist-get parsed-test :success)
                            (let ((cleaned (copy-sequence parsed-test)))
                              (plist-put cleaned :ast nil)
                              cleaned))
                           (t nil)))
             (test-ops (when clean-parsed
                        (jf/bash-analyze-file-operations-recursive
  ...
  (let ((operations nil))
    (when (fboundp 'jf/bash-parse)
      (let* ((parsed-then (jf/bash-parse then-text))
             ;; Clean :ast to avoid killed buffer issues
             (clean-parsed (cond
                           ((plist-get parsed-then :error)
                            (lwarn 'bash-parser :error "Then branch parse failed: %s"
                                   (plist-get parsed-then :error))
                            nil)
                           ((plist-get parsed-then :success)
  ...
  (let ((operations nil))
    (when (fboundp 'jf/bash-parse)
      (let* ((parsed-else (jf/bash-parse else-text))
             ;; Clean :ast to avoid killed buffer issues
             (clean-parsed (cond
                           ((plist-get parsed-else :error)
                            (lwarn 'bash-parser :error "Else branch parse failed: %s"
                                   (plist-get parsed-else :error))
                            nil)
                           ((plist-get parsed-else :success)
