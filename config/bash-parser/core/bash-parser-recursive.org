...
Extract focused helper functions for specific responsibilities.

*** Process Command Substitutions

Extract operations from command substitutions and track pattern output.
...
*** Process Command Chains with Context

Process command chains with variable and directory context tracking.
...
*** Process For-Loop with Binding

Process for-loop with variable binding in loop body.
...
The heart of the recursive semantic analyzer. This function orchestrates
extraction of file operations from a parsed command and delegates to helpers
...
** Pattern Flow Operations

Create operations for outer commands consuming patterns from substitutions.
This function is used to detect pattern flow - when a substitution produces
a file pattern (via find, ls, grep -l) and the outer command operates on
that pattern.
...
This is used by pattern flow analysis to determine what operation the outer
command performs on the pattern produced by a substitution.
