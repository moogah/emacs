...
**Version Pinning**: Python and JavaScript are pinned to specific versions (v0.21.0 and v0.21.4 respectively) to maintain ABI version 14 compatibility with Emacs 30.1. The master branches of these grammars have moved to ABI version 15, which causes version-mismatch errors.
...
*** Installing All Grammars
To install all configured Tree-sitter grammars at once:
...
To install only the most commonly used grammars (python, javascript, typescript, yaml, json):

#+begin_src emacs-lisp :tangle no
(dolist (lang '(python javascript typescript yaml json))
  (treesit-install-language-grammar lang))
#+end_src

Or install a single grammar:
...
#+begin_src emacs-lisp :tangle no
(treesit-language-available-p 'python)  ; Should return t
#+end_src

Check all configured grammars:

#+begin_src emacs-lisp :tangle no
(dolist (lang '(python javascript typescript yaml json))
  (message "%s: %s" lang (treesit-language-available-p lang)))
#+end_src

*** Troubleshooting

**** ABI Version Mismatch
If you see warnings like "version-mismatch: 15", the grammar ABI version doesn't match Emacs's expected version. This happens when grammar repositories update to newer tree-sitter versions.

Solution: Pin the grammar to a compatible version in ~treesit-language-source-alist~ (see version pinning notes above).

**** Missing C Compiler
Grammar installation requires a C compiler. Verify you have one:
...
**When to remove old grammars:**
- After changing version pins in ~treesit-language-source-alist~
- When encountering ABI version mismatch errors
- When grammar files become corrupted
- After updating Emacs to a new major version
- To force a fresh download and recompile

**Remove specific grammar:**

#+begin_src bash :tangle no
# macOS
rm ~/emacs/runtime/tree-sitter/libtree-sitter-python.dylib

# Linux
rm ~/emacs/runtime/tree-sitter/libtree-sitter-python.so

# Windows
rm ~/emacs/runtime/tree-sitter/libtree-sitter-python.dll
#+end_src

**Clean install - remove all grammars:**

#+begin_src bash :tangle no
# Remove all grammar files
rm -rf ~/emacs/runtime/tree-sitter/*

# Verify directory is empty
ls ~/emacs/runtime/tree-sitter/
#+end_src

**After removing grammars, reinstall:**

#+begin_src emacs-lisp :tangle no
;; Reinstall a single grammar
(treesit-install-language-grammar 'python)

;; Or reinstall commonly used grammars
(dolist (lang '(python javascript typescript yaml json))
  (treesit-install-language-grammar lang))

;; Or reinstall all configured grammars
(mapc #'treesit-install-language-grammar
      (mapcar #'car treesit-language-source-alist))
#+end_src

**Note:** The ~runtime/tree-sitter/~ directory is gitignored, so grammar files are not tracked in version control. Each worktree needs its own grammar installation.
