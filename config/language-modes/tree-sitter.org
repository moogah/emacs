...
Compiled grammars live in =runtime/tree-sitter/= (one per worktree, gitignored). This directory must be on =treesit-extra-load-path= so the built-in =treesit= API can find them. Without this, =treesit-parser-create= fails with "Cannot load language definition: not-found".

The bash-parser module (=config/bash-parser/=) depends on this for scope validation — if the bash grammar is missing, all =run_bash_command= tool calls will be denied.

New worktrees get grammars via =init-worktree-runtime.sh=. To install from scratch, see [[*Installation Instructions][Installation Instructions]] below.
...
./bin/emacs-isolated.sh --batch --eval "(progn \
  (let ((dir (expand-file-name \"runtime/tree-sitter/\" jf/emacs-dir))) \
...
;; Install all grammars (skips already-installed)
(let ((dir (expand-file-name "runtime/tree-sitter/" jf/emacs-dir)))
  (dolist (lang (mapcar #'car treesit-language-source-alist))
    (unless (treesit-language-available-p lang)
      (treesit-install-language-grammar lang dir))))
...
Grammars are compiled and installed to ~runtime/tree-sitter/~ as ~.dylib~ files (macOS), ~.so~ files (Linux), or ~.dll~ files (Windows). The =treesit-extra-load-path= setting (configured above) tells Emacs where to find them.
