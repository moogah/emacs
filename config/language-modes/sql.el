...
Returns port number or INTERNAL-PORT if container is not found or port not mapped."
  (let* ((cmd (format "docker port %s %d 2>/dev/null | cut -d: -f2"
    ...
    (if (string-empty-p output)
        (progn
          (message "Docker container %s not running; using default port %d."
...
;; Register connections just-in-time before org-babel SQL execution.
;; This avoids eager Docker port resolution at startup when containers
;; may not be running.  jf/sql-connect handles its own registration.
(with-eval-after-load 'ob-sql
  (advice-add 'org-babel-execute:sql :before
              (lambda (&rest _) (jf/sql-register-connections))))
