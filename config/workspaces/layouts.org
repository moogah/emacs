...
SLOT defaults to =:saved-state= for backward compatibility with any
caller that has not yet been updated to the explicit two-state model,
but every in-tree caller now passes its slot explicitly.
