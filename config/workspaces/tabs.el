...
;; `workspace-menu' is the `C-x w' front-door transient defined in
;; `workspaces-transient', which loads AFTER this module (and after
;; `config/transient.el' installs the straight `transient' override).
;; We must NOT `(require 'workspaces-transient)' here: that would invert
;; the load order.  `workspace--offer-birth-menu' calls the symbol at
;; interactive invoke time only, by which point the prefix is defined.
(declare-function workspace-menu "workspaces-transient" ())
...
(defun workspace--offer-birth-menu (context)
  "Raise the `workspace-menu' front door after a workspace is born.
CONTEXT is the workspace-creation context label (one of `fresh',
`anchored-scaffolded', or `anchored-existing', per
`register/vocabulary/workspace-creation-context').

The menu is offered ONLY for `fresh' and `anchored-scaffolded': both
are workspaces this package just scaffolded, so offering its
registry-driven Integrations group (e.g. add a git worktree) is
appropriate.  `anchored-existing' (adopted) workspaces are SKIPPED:
we never inject artifacts into a directory the user already owned
\(mirrors the gptel on-create `anchored-existing' skip in
`config/gptel/sessions/workspace-integration.org').

The new workspace is already current when this runs, so the menu's
Integrations group builds its `:menu' payload against it via
`workspace--integration-payload'
\(`register/shape/workspace-integration-anchor-payload').  We name no
integration — we only raise the generic front-door transient, keeping
`workspace-new' free of any consumer reference
\(`register/boundary/workspace-integration-registry').

A no-op in batch/non-interactive contexts (`noninteractive'): we never
launch a real transient under the test runner, and `workspace-menu'
may not even be defined there (it is established lazily under
`with-eval-after-load 'transient')."
  (when (and (memq context '(fresh anchored-scaffolded))
             (not noninteractive)
             (fboundp 'workspace-menu))
    (workspace-menu)))
...
workspace plist on success."
  (let* ((home (file-name-as-directory
    ...
    (let* ((is-repo? (file-directory-p (expand-file-name ".git" home)))
      ...
      ;; Offer the front-door menu (worktrees etc.) for scaffolded
      ;; births only.  `anchored-existing' is skipped inside the helper
      ;; — we never auto-open a menu against a directory the user owned.
      (workspace--offer-birth-menu context)
      (gethash name workspace--registry))))
...
never blocks the already-clean workspace (design Decision 4)."
  (let* ((home (expand-file-name
    ...
    ;; Offer the front-door menu (worktrees etc.) now that the fresh
    ;; workspace is current and registered (design.md D4).
