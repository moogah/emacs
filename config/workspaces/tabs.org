...
=find-file='s =<:home>/home.org= for it.  The fallback to =*scratch*=
is purely defensive: with the cycle-1 data-model change every
registered workspace has =:home= (=register/invariant/home-required-no-
floating-workspaces=), so the =if= branch should never fire in
practice.  Any code path that produces a no-=:home= workspace is a
separate bug to fix.
