...
Startup no longer creates a tab per workspace, and there is no lazy
per-workspace layout activation; the =:after= advice that drove it
(=workspace--persistence-after-tab-switch=) was removed.  The only
remaining workspaces tab-switch advice lives in =persistence.org=: the
=:before= autosave (=workspace--persistence-before-tab-switch= on
=tab-bar-switch-to-tab= / =tab-bar-select-tab=), which snapshots the
outgoing workspace's =:working-state=.  Layout is now applied only by
the explicit =workspace-restore= / =workspace-switch-layout= commands.
