...
a prefix arg it anchors an existing directory (implemented by sibling
task =workspace-new-anchor-existing=).  This cycle wires only the
default-path branch; the anchor-existing branch is a stub that signals
=user-error= so the interactive call structure is in place for the
follow-up cycle.
