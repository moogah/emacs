...
Two extension surfaces exist per integration:
...
surface (=:on-create= or =:menu=); re-registering an ID overwrites its
plist *in place*, preserving order.
...
Runs one entry's =:on-create= under an error guard and normalizes the
