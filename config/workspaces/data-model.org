...
* Restore-pending runtime tag

The =:restore-pending= tag is a *runtime-only* flag set by the
persistence loader when a workspace is read into the registry but
its recent-layout frameset has not yet been applied (lazy
activation).  It is never serialized — the next save writes the
workspace without it, since the tag's job is exhausted once the
tab-switch hook fires =workspace--apply-saved-layout=.

Like =:broken=, mutations of this tag MUST route through the
named helpers below — never inline =(plist-put copy
:restore-pending …)=.  The architect's
=arch-cycle-20260525-200459-2= (cycle 1) established the
helper-vs-inline contract for =:broken=;
=arch-cycle-20260525-213500-03= (cycle 2) extended it to
=:restore-pending=.  Future runtime-only tags follow the same
pattern (a candidate invariant
=register/invariant/runtime-tags-routed-through-helpers= will
