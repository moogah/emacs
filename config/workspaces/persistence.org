...
Per-machine on-disk persistence for workspaces.  Every trigger flushes
synchronously through =workspace--flush-state= — auto-save on layout
context switch, intra-workspace layout switch, idle tick, and Emacs
shutdown, plus explicit =workspace-save=.  There is no debounce (see
"Synchronous save").  Restore on startup populates the registry and
re-creates tabs without disturbing the active tab; layout/buffer restore
is deferred to the first selection of each restored workspace (lazy
activation).
...
Every persistence trigger flushes synchronously through this one entry
point.  There is no debounce: every trigger in the package is a discrete,
low-frequency, quiescent event (explicit save, layout save/switch, tab
switch, idle tick, shutdown) with no burst to coalesce, so a debounced
idle-timer write would only delay the on-disk write for no benefit.  A
debounce would be warranted only for a high-frequency trigger such as
autosave on window-configuration change, which is not part of this
package; should such a trigger be added it would re-introduce a debounce
scoped to itself.  The block check, readable-by-construction assert, and
atomic temp-file+rename all live in =workspace--write-state=.
