...
(defun jf/bash--static-dirname (path)
  "Static evaluation of dirname command on PATH.

Returns the directory component of PATH using Emacs file-name-directory.
This is the static equivalent of the bash dirname command.
...
Note: Returns empty string for bare filenames without directory component."
...
(defun jf/bash--static-basename (path &optional suffix)
  "Static evaluation of basename command on PATH, optionally removing SUFFIX.

Returns the filename component of PATH using Emacs file-name-nondirectory.
If SUFFIX is provided and PATH ends with it, removes the suffix.

This is the static equivalent of the bash basename command.
...
Examples:
  ...
  (let* ((command-name (plist-get command :command-name))
    ...
    (cond
     ;; No target provided (pushd with no args) - complex directory swap, skip for now
     ((null target)
      :unresolved)
     ...
     ;; pushd ~/path (expand ~ to HOME, then append path)
     ((string-prefix-p "~/" target)
      (if home
          (let ((remainder (substring target 2)))  ; Remove "~/" prefix
            (expand-file-name (concat home "/" remainder)))
     ...
     ;; Have a regular target - resolve it (same logic as cd)
     (t
      ;; Step 1: Resolve variables
      (let ((var-resolved (jf/bash-resolve-variables target var-context)))
        (cond
         ...
         ;; Variables resolved - continue with path resolution
         (t
          ;; Step 2: Resolve command substitutions
          (let ((cmd-resolved (jf/bash-resolve-command-substitution var-resolved var-context)))
            (cond
             ;; Command substitution could not be resolved
             ((eq cmd-resolved :unresolved)
