...
The orchestrator coordinates plugin execution to extract semantic meaning from parsed commands.
...
The filesystem plugin wraps existing filesystem extraction logic in the plugin protocol.
This validates the plugin architecture by wrapping a mature, tested extraction subsystem.
...
The plugin must identify which tokens it understood during extraction.

*** Helper: Find Tokens for File Path
...
*** Helper: Claim Tokens for Redirection
...
*** Helper: Claim Tokens for Operations
...
*** Has File-Related Tokens Predicate
...
Register the filesystem plugin with priority 100 (universal predicates run early).
