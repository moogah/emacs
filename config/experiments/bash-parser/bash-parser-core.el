...
(require 'treesit)
(require 'cl-lib)
(require 'bash-parser-protocol)  ; Forward declarations
(require 'bash-parser-security)

(defvar jf/bash-parse--token-counter 0
  "Counter for generating sequential token IDs during parsing.
Reset to 0 at the start of each parse operation.")

(defvar jf/bash-parse--current-pipe-segment 0
  "Current pipeline segment number during parsing.
Bound dynamically to track which segment of a pipeline is being parsed.")

(defun jf/bash-parse--make-token (type value start end pipe-segment &optional associated-flag parsed-inner)
  "Create a token plist with all required fields.
TYPE is a keyword like :command-name, :positional-arg, :flag, etc.
VALUE is the string content of the token.
START is the 0-indexed character position where token begins.
END is the exclusive character position after the last character.
PIPE-SEGMENT is the integer pipeline segment number (0 for non-pipeline).
ASSOCIATED-FLAG is optional token ID of associated flag (for flag-args).
PARSED-INNER is optional boolean indicating if command substitution was parsed.
Returns plist with :id, :type, :value, :start, :end, :pipe-segment, and optional fields."
  (let ((token-id jf/bash-parse--token-counter))
    (setq jf/bash-parse--token-counter (1+ jf/bash-parse--token-counter))
    (let ((token (list :id token-id
                      :type type
                      :value value
                      :start start
                      :end end
                      :pipe-segment pipe-segment)))
      (when associated-flag
        (setq token (plist-put token :associated-flag associated-flag)))
      (when parsed-inner
        (setq token (plist-put token :parsed-inner parsed-inner)))
      token)))

(defvar jf/bash-parse--current-tokens nil
  "List of tokens collected during current parse operation.
Bound dynamically during parsing to accumulate tokens.")

(defun jf/bash-parse--emit-token (type value start end pipe-segment &optional associated-flag parsed-inner)
  "Emit a token by creating it and adding to current token list.
See `jf/bash-parse--make-token' for parameter descriptions."
  (let ((token (jf/bash-parse--make-token type value start end pipe-segment associated-flag parsed-inner)))
    (push token jf/bash-parse--current-tokens)
    token))

(defun jf/bash-parse--finalize-tokens ()
  "Return collected tokens sorted by position.
Reverses the accumulated token list to restore parse order."
  (nreverse jf/bash-parse--current-tokens))

(defun jf/bash-parse--emit-command-tokens (command-node command-name args pipe-segment)
  "Emit tokens for a command's components.
COMMAND-NODE is the tree-sitter command node.
COMMAND-NAME is the command name string.
ARGS is the list of argument strings.
PIPE-SEGMENT is the current pipeline segment number.
Emits :command-name token, then :flag/:positional-arg tokens for each arg."
  ;; Find command_name node to get position
  (when-let ((cmd-name-node (jf/bash-parse--find-direct-child-by-type command-node "command_name")))
    (jf/bash-parse--emit-token :command-name
                               command-name
                               (treesit-node-start cmd-name-node)
                               (treesit-node-end cmd-name-node)
                               pipe-segment))

  ;; Emit tokens for each argument by finding corresponding nodes
  ;; We need to traverse the command node to find word nodes for each arg
  (let ((arg-index 0)
        (args-to-find (copy-sequence args)))
    (jf/bash-parse--visit-node
     command-node
     (lambda (node)
       (let ((node-type (treesit-node-type node)))
         (cond
          ;; Skip command_name - already handled
          ((string= node-type "command_name")
           :skip-children)
          ...
          ;; Check word/string/concatenation nodes for matching args
          ((and args-to-find
           ...
           (let ((text (treesit-node-text node t)))
             (when text
               ...
               ;; Strip quotes from string types
               (when (and (> (length text) 1)
                         (member node-type '("string" "raw_string"))
                         (or (and (string-prefix-p "\"" text)
                                  (string-suffix-p "\"" text))
                             (and (string-prefix-p "'" text)
                                  (string-suffix-p "'" text))))
                 (setq text (substring text 1 -1)))
               ;; Check if this matches next expected arg
               (when (and args-to-find (string= text (car args-to-find)))
                 (let ((token-type (if (string-prefix-p "-" text) :flag :positional-arg)))
                   (jf/bash-parse--emit-token token-type
                                              text
                                              (treesit-node-start node)
                                              (treesit-node-end node)
                                              pipe-segment))
                 (setq args-to-find (cdr args-to-find))))
             ;; Skip children of terminal nodes
             (when (member node-type '("string" "raw_string" "concatenation" "simple_expansion" "expansion"))
               :skip-children)))
...
KNOWN LIMITATIONS:
  ...
  (if (string-empty-p command-string)
    ...
    (let ((jf/bash-parse--token-counter 0)
          ...
          (jf/bash-parse--current-pipe-segment 0))
      ;; Proceed with normal parsing
      (let ((result (jf/bash-parse--with-depth command-string 0)))
        ;; Add finalized tokens to result
        (when (plist-get result :success)
          (setq result (plist-put result :tokens (jf/bash-parse--finalize-tokens))))
        result))))
...
DEPTH parameter tracks current recursion depth."
  (let* ((pipeline-node (jf/bash-parse--find-node-by-type root-node "pipeline"))
    ...
    ;; Emit tokens for pipe operators
    (when pipeline-node
      (jf/bash-parse--visit-node
       pipeline-node
       (lambda (node)
         (when (string= (treesit-node-type node) "|")
           (jf/bash-parse--emit-token :pipe
                                      "|"
                                      (treesit-node-start node)
                                      (treesit-node-end node)
                                      jf/bash-parse--current-pipe-segment))
         nil)))
    ;; Parse commands with pipe segment tracking
    (let* ((parsed-commands
            (cl-loop for node in command-nodes
                     for pipe-seg from 0
                     collect (let ((jf/bash-parse--current-pipe-segment pipe-seg))
...
DEPTH parameter tracks current recursion depth."
  (let* ((list-node (jf/bash-parse--find-node-by-type root-node "list"))
    ...
    ;; Emit tokens for chain separators (&&, ||, ;)
    (when container-node
      (jf/bash-parse--visit-node
       container-node
       (lambda (node)
         (let ((node-type (treesit-node-type node)))
           (when (member node-type '("&&" "||" ";"))
             (jf/bash-parse--emit-token :separator
                                        node-type
                                        (treesit-node-start node)
                                        (treesit-node-end node)
                                        jf/bash-parse--current-pipe-segment)))
         nil)))
    (let* ((command-nodes (jf/bash-parse--get-all-command-nodes container-node))
     ...
     (t
      ...
      (if (string= (treesit-node-type command-node) "variable_assignment")
        ...
        (let* ((env-vars (jf/bash-parse--extract-env-vars command-node))
               ;; Use dynamic pipe segment if set, otherwise default to 0
               (pipe-segment jf/bash-parse--current-pipe-segment)
               (words (jf/bash-parse--extract-words command-node pipe-segment))
          ...
          (if-let ((wrapper-spec (and (not (string-match-p "=" command-name))
            ...
            (if (and (string= command-name "find")
              ...
              (let* ((flags (jf/bash-parse--extract-flags args-start))
                ...
                ;; Emit tokens for this command
                (jf/bash-parse--emit-command-tokens command-node command-name args-start pipe-segment)
...
(defun jf/bash-parse--extract-words (command-node &optional pipe-segment)
...
PIPE-SEGMENT is the current pipeline segment number (0 for non-pipeline).
Emits tokens for each word extracted."
  (let ((words '())
        (pipe-seg (or pipe-segment 0)))
    (jf/bash-parse--visit-node
     command-node
     (lambda (node)
       (let ((node-type (treesit-node-type node)))
           ...
           (let ((text (treesit-node-text node t))
             ...
             (when (and text (> (length text) 0))
               ...
               (unless (string-empty-p text)
                 ;; Emit token for command substitution
                 (when (string= node-type "command_substitution")
                   (jf/bash-parse--emit-token :command-substitution text start end pipe-seg nil t))
                 (push text words))))
...
Emits :redirection tokens for each redirection found."
  (let ((redirects '()))
    (jf/bash-parse--visit-node
     statement-node
     (lambda (node)
       (let ((node-type (treesit-node-type node)))
         (cond
          ...
          ((string= node-type "file_redirect")
           (let ((redirect-plist (jf/bash-parse--parse-file-redirect node)))
             ;; Emit token for the redirection
             (jf/bash-parse--emit-token :redirection
                                        (treesit-node-text node t)
                                        (treesit-node-start node)
                                        (treesit-node-end node)
                                        jf/bash-parse--current-pipe-segment)
             (push redirect-plist redirects)))

          ;; herestring_redirect: <<<
          ((string= node-type "herestring_redirect")
           (let ((redirect-plist (jf/bash-parse--parse-herestring-redirect node)))
             (jf/bash-parse--emit-token :redirection
                                        (treesit-node-text node t)
                                        (treesit-node-start node)
                                        (treesit-node-end node)
          ...
          ((string= node-type "heredoc_redirect")
           (let ((redirect-plist (jf/bash-parse--parse-heredoc-redirect node)))
             (jf/bash-parse--emit-token :redirection
                                        (treesit-node-text node t)
                                        (treesit-node-start node)
                                        (treesit-node-end node)
