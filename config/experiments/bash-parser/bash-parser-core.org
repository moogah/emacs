...
*** Double-Paren Subshell Parsing

Tree-sitter-bash has a parser bug where (( at the start of a command is
incorrectly parsed as arithmetic expansion $((...)) instead of nested
subshells ( (...)).

Workaround: The parser normalizes "((cmd))" to "( (cmd))" by adding a
space after the first paren. This allows tree-sitter to correctly parse
it as nested subshells.

Impact:
- Commands starting with (( parse correctly
- Position offsets in error reporting may be off by 1 for these commands
- No functional impact on command extraction or analysis

Tracking:
- TODO: Remove workaround when tree-sitter-bash fixes (( parsing
- Upstream issue: https://github.com/tree-sitter/tree-sitter-bash/issues/XXX
  (Note: Issue number placeholder - actual issue may not exist yet)
- Test coverage: test-directory-changing-commands.el test-nested-subshells

Related code:
- Workaround implementation: jf/bash-parse--internal (lines 129-136)
- Test case: test-nested-subshells in test-directory-changing-commands.el
...
Global counter for generating sequential token IDs during parsing.
...
** Token Emission for Command Components
...
Detect commands that inject another command as an argument (e.g., bash -c, sh -c, env -S).
These patterns allow arbitrary command execution within a string argument.
