...
- Initial directory paths (before any expression flags)
- Expression flags like -name, -type, -path that consume the next argument
- Action flags like -exec that have their own argument structure

We need to extract only the initial directory arguments and classify them as
:read-directory operations, ignoring flag arguments.
