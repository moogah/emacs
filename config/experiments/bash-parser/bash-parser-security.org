#+title: Bash Parser - Security Validation
#+author: Jeff Farr
#+property: header-args:emacs-lisp :tangle bash-parser-security.el
#+auto_tangle: t
...
This module implements sandbox security validation that controls which file
operations are allowed on which paths. It includes:
- Dangerous command pattern detection
- Wrapper command handling (sudo, env, time, etc.)
- Security rule matching with first-match semantics
- Operation permission checking
- CD command detection and blocking
...
Validate that an operation is allowed by a matched rule.
