#+title: Bash Parser - Variable Context Management
#+author: Jeff Farr
#+property: header-args:emacs-lisp :tangle bash-parser-variables.el
#+auto_tangle: t
...
This module handles variable references in file paths, enabling scope-aware
file operations. It supports:
- Variable reference detection ($VAR, ${VAR})
- Variable resolution against provided context
- Partial resolution (some variables resolved, others remain)
- Variable assignment tracking in command chains
...
Detect variable references in file paths without resolving them.
...
Resolve relative paths (., ./, ../) against PWD from variable context.
...
** Command Substitution Resolution

Resolve command substitutions like $(pwd), $(dirname $VAR), and `pwd` using static evaluation.

*** Static Command Evaluation Helpers

Helper functions for static evaluation of deterministic commands.
...
*** General Command Substitution Resolution

Resolve command substitutions with static evaluation for deterministic commands.
...
Helper function to resolve assignment values using the same logic as file paths.
...
Detect if a parsed command structure represents a cd command.
...
** CD Target Extraction and Resolution

Extract target directory from cd command and resolve it to an absolute path.
...
** pushd Command Detection

Detect if a parsed command structure represents a pushd command.
...
** popd Command Detection

Detect if a parsed command structure represents a popd command.
...
** pushd Target Extraction and Resolution

Extract target directory from pushd command and resolve it to an absolute path.
