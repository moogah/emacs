...
This module provides filesystem utilities for the gptel session browser.
It implements a directory-based tree structure where:
- Each message or response is a directory
- Each directory contains context.md (complete conversation to that point)
- Optional tools.md for tool calls
- The =current= symlink points to the active conversation position

Directory structure example:
#+begin_example
~/gptel-sessions/SESSION_ID/
├── current -> msg-1/response-1/context.md
├── metadata.json
└── msg-1/
    ├── context.md
    ├── tools.md (optional)
...
Track the current position in the conversation tree using a path structure.
...
Build complete conversation context from conversation history.
...
Create directories and files for conversation tree nodes.
...
Save tool calls to tools.md companion file.
...
Manage the =current= symlink that points to the active conversation position.
...
Create initial session directory structure.
