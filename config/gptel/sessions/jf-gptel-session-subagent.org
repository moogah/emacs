#+title: GPTEL Session Subagent Integration
#+author: Jeff Farr
#+property: header-args:emacs-lisp :tangle jf-gptel-session-subagent.el
#+auto_tangle: y

* Introduction

Subagent session integration for gptel persistent sessions.
Captures subagent executions and creates nested session directories with bidirectional linking.
...
Create a session directory for a subagent under the parent session.
...
Create metadata file for subagent with parent link.
...
Advice the Agent tool to create subagent sessions and capture their conversations.
...
* Enhanced Tool Logging for Subagents

Extend tool logging to include subagent session links.
