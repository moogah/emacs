...
- Legacy support: reads from scope-plan.yml and preset.md for older sessions
...
Metadata is stored in two separate files:

**scope-plan.yml** contains session-level fields:
...
**preset.md** contains configuration fields in YAML frontmatter:
...
Functions for reading session metadata from scope-plan.yml and preset.md files.
