...
- :preset-file - Path to external preset file (.md or .org format)
  When present, session loads preset from this file instead of
  inline preset.md/preset.org in session directory.
  Allows multiple sessions to share the same preset.
