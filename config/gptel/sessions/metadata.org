...
This approach avoids data duplication - scope-plan.yml already contains session metadata,
and preset.md already contains configuration settings.
...
- type - Session type ("subagent" for subagent sessions, omitted for regular sessions)
- parent_session_id - Parent session ID (for subagents only)
- agent_type - Agent type (e.g., "Bash", "Explore" - for subagents only)
...
- :session-id, :created, :updated, :type, :parent-session-id, :agent-type
- :backend, :model
