...
- type - Session type ("agent" for agent sessions, omitted for regular sessions)
- parent_session_id - Parent session ID (for agents only)
- agent_type - Agent type (e.g., "Bash", "Explore" - for agents only)
