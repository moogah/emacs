...
** Session Headings Block Helper

Single source of truth for the =* System Prompt= / =* Chat=
heading shape introduced by design.md §Addendum (Finding B /
Decision B). The creation renderer and the save-path materialiser
(=make-system-prompt-heading-authoritative=) both build the
heading shape through this helper so the literal heading strings
and the =:VISIBILITY: folded= property are not duplicated across
modules (=register/shape/session-document-layout=).

The =* System Prompt= heading carries a =:PROPERTIES:= /
=:VISIBILITY: folded= / =:END:= drawer so org folds the subtree
on open (Decision C); its body is seeded from the active preset's
=:system= text. When the preset has no =:system=, the heading is
still emitted with an empty body — the document shape stays
canonical and consistent (=register/shape/session-document-layout=
...
=session.org= — the =* System Prompt= heading (preset-seeded,
folded) and the =* Chat= heading holding the empty user block
template (design.md §Addendum Decision B; Decision 9 / Decision
18). The file-level config drawer is rendered separately by
=jf/gptel-scope-profile--render-drawer-text= (Mode 2a) and
prepended at the call site, since the drawer is the authoritative
carrier for both =GPTEL_PRESET= / =GPTEL_PARENT_SESSION_ID= AND
the resolved =GPTEL_SCOPE_*= keys (gptel-scope-in-org-properties:
...
5. Creates session directory structure:
   - =session.org= pre-populated with a file-level =:PROPERTIES:=
     ...
     =GPTEL_PARENT_SESSION_ID=, and =:GPTEL_SCOPE_*:= keys rendered
     from the preset's scope profile (via
     =jf/gptel-scope-profile--create-for-session= Mode 2a),
     followed by a folded =* System Prompt= heading (seeded from
     the preset's =:system= text) and a =* Chat= heading holding
     the chat-mode empty-user-block template (Decision 4 /
     Decision 9 / §Addendum Decision B). No =metadata.yml= sidecar
     and no =scope.yml= file are written — the drawer is
     authoritative (Decision 6; gptel-scope-in-org-properties
     drawer-resident scope).
