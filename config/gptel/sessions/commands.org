...
5. Creates session directory structure:
   ...
   - =session.org= with chat-mode initial content (an empty
     =#+begin_user= / =#+end_user= block — Decision 9 / Decision 18),
     so a fresh session looks identical to a fresh standalone
     =gptel-chat-mode= buffer
6. Opens session buffer - auto-initialization hook applies preset on
   first open.  The session file format is self-describing (block
   delimiters), so there is no =gptel--save-state= / Local Variables
   round-trip; =metadata.yml= remains the authoritative configuration
   source (Decision 16 / Decision 18).
