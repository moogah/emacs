...
Interactive command to create a new persistent session with optional projectile integration.
...
7. Opens session buffer

** Projectile Integration (New)
...
Interactive command to create a programming session with scoped tools and projectile integration.
This uses the programming assistant preset template which includes:
