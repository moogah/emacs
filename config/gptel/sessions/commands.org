...
** Replace Drawer Property

Branch creation copies the parent's drawer verbatim, so the new
branch's drawer arrives carrying the parent's =:GPTEL_BRANCH:= value.
That value must be OVERWRITTEN with the new branch name — appending
would leave two =:GPTEL_BRANCH:= lines, violating both
=register/shape/drawer-text-block= (one key-line per property) and
=register/invariant/branch-drawer-shares-id-not-branch= (no duplicate
key survives).  This helper performs an in-place value REPLACE on an
existing key-line inside the =:PROPERTIES:= / =:END:= block, falling
back to an append (via =jf/gptel--append-drawer-property=) when the
key is absent so a parent drawer that predates identity-key emission
still gains the key.
