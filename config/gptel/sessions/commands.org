...
The mode-side counterpart to the path-level =find-file-hook= above.
...
How it differs from the path-level auto-init hook:
...
(mirroring the old auto-init isolation).
