...
** Initial Session Content Helper (legacy)

Drawer + body composition retained for backward compatibility with
direct callers and the helper-level tests. =--create-session-core=
no longer routes through this function; instead it composes the
drawer text from =--render-drawer-text= (which now also carries the
=GPTEL_SCOPE_*= keys) with =--initial-session-body=. This helper
synthesises an equivalent drawer with =GPTEL_PRESET= and optional
=GPTEL_PARENT_SESSION_ID= but no scope keys — adequate for tests
that pin the standalone drawer-shape contract.
