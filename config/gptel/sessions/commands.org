...
└── my-feature-20260130/
    ...
    │   │       │                     # PROPERTIES drawer holds preset name
...
- Ordering: =jf/gptel--ensure-mode-once= runs *before* the
  =metadata.yml= apply. This guarantees the chat-mode hook
  (=gptel-chat--apply-declared-preset=, installed on
  =gptel-chat-mode-hook= in =config/gptel/chat/menu.org=) fires first
  and applies any drawer/file-local preset; the auto-init hook then
  applies =metadata.yml='s preset last so it wins. (Auto-init runs on
  =find-file-hook=, after =set-auto-mode= and
  =hack-local-variables-hook= have already fired.)
...
** Initial Session Content Helper

Builds the pre-populated =session.org= content at creation time
(design Decision 4). The drawer shape matches what the chat-mode
save hook would produce on first save, so creation-then-save
converges deterministically on the same on-disk format.

Raw string concatenation is intentional: this is a write to a
fresh file on disk, not a live buffer. Using =org-entry-put= (the
runtime drawer writer) would require activating a buffer and
org-mode, which is far heavier than a single =format= call.
...
5. Creates session directory structure:
   ...
   - =session.org= pre-populated with a =:PROPERTIES:= drawer containing
     ...
     (Decision 4 / Decision 9). No =metadata.yml= sidecar is written —
     the drawer is authoritative (Decision 6).
