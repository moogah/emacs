...
=session.org= — the empty user block template that follows the
=:PROPERTIES:= drawer (Decision 9 / Decision 18). The drawer
itself is rendered separately by
=jf/gptel-scope-profile--render-drawer-text= (Mode 2a) and
prepended at the call site, since the drawer is now the
...
5. Creates session directory structure:
   - =session.org= pre-populated with a =:PROPERTIES:= drawer
     containing =GPTEL_PRESET=, optional =GPTEL_PARENT_SESSION_ID=,
     and =:GPTEL_SCOPE_*:= keys rendered from the preset's scope
     profile (via =jf/gptel-scope-profile--create-for-session= Mode
     2a), followed by the chat-mode empty-user-block template
     (Decision 4 / Decision 9). No =metadata.yml= sidecar and no
