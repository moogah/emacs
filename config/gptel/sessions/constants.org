...
Default location for preset template files.
...
* Scope Profiles Directory Configuration

Default location for scope profile template files.
...
Directory where scope profile templates are stored.
...
Standard file name for scope configuration written into session directories.
