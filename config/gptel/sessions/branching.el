...
(defun jf/gptel--session-file-format (buffer-or-file)
  "Detect session file format from BUFFER-OR-FILE. Returns :org or :md."
...
(defun jf/gptel--get-session-file-extension (buffer)
  "Get session file extension string from BUFFER."
  (file-name-extension (buffer-file-name buffer)))

(defun jf/gptel--write-local-variables (format vars)
  "Write Local Variables block in FORMAT (:org or :md) with VARS alist.
Inserts at point. FORMAT is :org or :md. VARS is an alist of (symbol . value)."
  (pcase format
    (:org
     (insert "# Local Variables:\n")
     (dolist (var vars)
       (insert (format "# %s: %s\n" (car var) (prin1-to-string (cdr var)))))
     (insert "# End:\n"))
    (:md
     (insert "<!-- Local Variables: -->\n")
     (dolist (var vars)
