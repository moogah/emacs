...
Detects session files by path pattern:
  - Branch sessions: */branches/<branch-name>/session.org
  - Agent sessions:  */branches/<branch-name>/agents/<agent-name>/session.org
                     (fallback: */agents/<agent-name>/session.org)
...
critical."
  ...
  (when (and (buffer-file-name)                         ; Has file?
    ...
    (let* ((file-path (expand-file-name (buffer-file-name)))
      ...
      (when (and (string= file-name "session.org")
      ...
      ;; Agent session: */agents/<agent>/session.org
      ;; (Matches both the per-branch layout */branches/<branch>/agents/<agent>/session.org
      ;;  and the legacy flat */agents/<agent>/session.org layout.)
      (when (and (not session-type)
                 (string= file-name "session.org")
                      ...
                      (condition-case parse-err
                          (with-temp-buffer
                            ...
                            (let* ((parsed (yaml-parse-string (buffer-string)
                              ...
                              (when (and preset-str (stringp preset-str))
                                (setq preset-name (intern preset-str)))))
                        (error
                         (jf/gptel--log 'warn "Failed to read metadata.yml: %s"
