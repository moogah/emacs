...
;; the session.org `:PROPERTIES:' drawer. Every time the session file is
;; opened, `gptel-chat-mode' activation reads the drawer and applies the
;; declared preset (plus overlaying non-preset deltas and
;; `GPTEL_PARENT_SESSION_ID') via `gptel-chat--apply-declared-preset'.
;; The drawer is the authoritative configuration source for session
;; buffers — they run `gptel-chat-mode' exclusively (Decision 16) and do
;; NOT round-trip through gptel--save-state / gptel--restore-state.
