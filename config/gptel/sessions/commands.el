...
- session.org pre-populated with the drawer-resident scope (preset
  ...
  PROJECT-ROOT verbatim — byte-identical to the string substituted
  into the `${project_root}' scope keys, so the two agree by
...
Returns plist with:
  ...
  (let* ((main-branch-dir (jf/gptel--create-branch-directory session-dir "main"))
         ...
         ;; Stored VERBATIM — the byte-identical PROJECT-ROOT string that
         ;; `jf/gptel-scope-profile--expand-string' substitutes into the
         ;; scope patterns (a raw `replace-regexp-in-string', NOT
         ;; `expand-file-name'; scope-profiles.org §expand-string).  Both
         ;; outputs are therefore the SAME string, so the session's working
         ;; directory and its scope boundary CANNOT disagree — the cwd↔scope
         ;; agreement invariant (register/invariant/cwd-scope-agreement) is
         ;; structural by construction (design.md D1), for ANY input form
         ;; (relative, trailing-slash, ~) rather than only canonical-absolute
         ;; ones.  Normalisation to an absolute directory happens at READ
         ;; time in the binder (`expand-file-name' + `file-name-as-directory'),
         ;; not here.  When PROJECT-ROOT is nil this is a keyless session: the
         ;; key is SKIPPED so the binder falls back to `branch-dir'.  Spliced
         ;; via the append helper to preserve the `:PROPERTIES:' / `:END:'
         ;; adjacency invariant.
