...
;; GPTEL_PRESET (and GPTEL_PARENT_SESSION_ID for agents). The drawer is
;; the authoritative session-level configuration source — gptel-chat-mode
;; reads it on buffer activation and writes it back via the save hook
;; (design Decision 4 / Decision 6). Session creation no longer writes a
;; metadata.yml sidecar; all session-level state is either path-derivable
;; or drawer-derived. Session buffers run gptel-chat-mode exclusively
;; (Decision 16).
