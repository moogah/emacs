...
Session branching allows you to create alternate conversation paths from any point
in the conversation history. This is useful for:
- Exploring different approaches to a problem
- Trying alternative phrasings of a question
- Comparing responses from different prompts
- Maintaining multiple conversation threads

Each branch is tracked in the conversation tree structure, and you can switch between
branches using the session browser.

This module provides:
- Branch creation from any point in conversation
- Branch candidate building for selection
- Branch buffer creation with context
- Next branch ID generation (handles -alt1, -alt2, etc.)
