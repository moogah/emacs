...
Session branching in the filesystem-based approach is done by copying and editing
context.md files. The workflow:

1. Navigate to any response-N directory in dirvish/dired
2. Copy the next msg-N directory to msg-N-alt1
3. Edit msg-N-alt1/context.md to change the message
4. Resume from the edited context and send

This module provides:
- Session resumption from context.md files
- Context file sending to API
- Branch creation helpers
- Node path determination from filesystem structure
...
* Resume Session from Context

Load a context.md file into a gptel buffer and set up session state for continuation.
...
Send an edited context.md file to the API to generate a response.
...
Create a branch by copying a node directory.
...
Walk up directory tree to find session root (contains metadata.json).
...
Determine the node path from a context file path.
