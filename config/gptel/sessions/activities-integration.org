...
- gptel session in =session/branches/main/session.org=
...
- Conversation history fully restored from =session.org=
...
└── implement-auth-20260130/
    ...
    │   ├── branches/
    │   │   └── main/
    │   │       └── session.org       # Your conversation (chat-mode)
...
- gptel session modules (registry, filesystem)
