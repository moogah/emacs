...
** Capture Tool Calls After Execution

This advice wraps gptel--handle-tool-use to capture tool calls after they've executed,
when we have complete information (ID, name, args, result) in one place.
