...
(let* ((test-dir (file-name-directory (or load-file-name buffer-file-name)))
  ...
  ;; Load scope modules for gptel-make-scoped-tool macro and validators
...
(defun run-bash-spec--get-tool (&optional name)
  "Find the registered tool named NAME (default \"run_bash_command\").
Searches across all categories in `gptel--known-tools' — matches how
gptel itself resolves tools by name."
  (cl-loop for (_ . tools) in gptel--known-tools
           thereis (alist-get (or name "run_bash_command")
                              tools nil nil #'equal)))

(defun run-bash-spec--get-tool-fn ()
  "Get the registered run_bash_command tool function."
  (when-let* ((tool (run-bash-spec--get-tool)))
...
(describe "run_bash_command: behavioral contract"
  ...
  (describe "tool infrastructure"
    ...
    (it "run_bash_command is registered in gptel--known-tools as async"
      (let ((tool (run-bash-spec--get-tool)))
  ...
  (describe "after user approves via expansion UI"
    ...
    (before-each
      ...
      ;; Mock expansion UI: immediately invokes wrapper-callback with
      ;; allow-once approval (:allowed-once skips re-validation so the
      ;; tool body runs once without looping back through the mocked
      ;; validator that would reject again).
      (spy-on 'jf/gptel-scope--trigger-inline-expansion
              :and-call-fake
              (lambda (_validation-error _tool-name wrapper-callback)
                (funcall wrapper-callback
                         (list :approved t :allowed-once t)))))
