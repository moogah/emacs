  ...
  (let ((where (point-marker)))
    ...
    (let* ((session-dir (jf/gptel--create-agent-directory
      ...
      (let* ((preset-plist (jf/gptel--load-preset-from-file session-dir)))
        ...
        ;; Get activity org-file from parent's scope-plan.yml
        ;; Parse worktrees from authoritative source (activity org-roam document)
        (let* ((parent-scope-file (jf/gptel--scope-plan-file-path jf/gptel--session-dir))
               (activity-org-file (when (file-exists-p parent-scope-file)
                                   (jf/gptel-scope--get-activity-org-file parent-scope-file)))
               (worktree-paths (when activity-org-file
                                (jf/gptel-activities--parse-worktree-paths
                                 nil  ; activity-name not needed when org-file provided
                                 activity-org-file)))
               ;; Generate scope plan using preset tools + worktree paths from activity
               (scope-yaml (jf/gptel--generate-scope-plan-yaml
                           ...
                           worktree-paths))  ; From activity org-roam document
               (scope-file (expand-file-name "scope-plan.yml" session-dir)))
          (with-temp-file scope-file
            (insert scope-yaml))
          (jf/gptel--log 'info "Created agent scope plan: %s%s"
                        scope-file
                        (if worktree-paths
                            (format " with %d worktree(s) from activity" (length worktree-paths))
                          "")))
