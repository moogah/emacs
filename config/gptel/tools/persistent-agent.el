        ...
        (let ((metadata (or (jf/gptel--read-session-metadata session-dir)
          ...
          (let* ((buffer-name (format "*gptel-agent:%s:%s*" preset description))
            ...
            (with-current-buffer agent-buffer
              ...
              ;; Agents are simple sessions without branching - use "main" as default branch
              (setq-local jf/gptel--branch-name "main")
              (setq-local jf/gptel--branch-dir session-dir)  ; For non-branching sessions, branch-dir = session-dir
