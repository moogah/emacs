...
(gptel-make-tool
 ...
 :function (lambda (filepath position)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath)))
                     (if (not language)
                         (format "Error: Unsupported file type for %s" filepath)
                       (if (not (treesit-language-available-p language))
                           (format "Error: Parser for %s not installed. Use check_treesitter_parser(\"%s\")"
                                   language filepath)
                         (let* ((parser (treesit-parser-create language))
                           ...
                           (if (not node)
                               (format "No node found at position %s" position)
                             (format "Node at Position %s:\n\nFile: %s\nLanguage: %s\n\nNode Type: %s\nStart: Line %d, Col %d\nEnd: Line %d, Col %d\nParent Type: %s\n\nNode Text:\n%s\n\nNext steps:\n- Use get_node_info(\"%s\", %s) for detailed analysis\n- Use get_node_context(\"%s\", %s) to see surrounding code"
                                     position filepath language
                                     (treesit-node-type node)
                                     (line-number-at-pos (treesit-node-start node))
                                     (current-column)
                                     (line-number-at-pos (treesit-node-end node))
                                     (current-column)
                                     (if (treesit-node-parent node)
 ...
 :description "Get AST node at a specific position in the file.
...
(gptel-make-tool
 :name "get_node_info"
 :function (lambda (filepath position &optional show-children)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath)))
                     (if (not language)
                         (format "Error: Unsupported file type for %s" filepath)
                       (if (not (treesit-language-available-p language))
                           (format "Error: Parser for %s not installed" language)
                         (let* ((parser (treesit-parser-create language))
                                (root-node (treesit-parser-root-node parser))
                                (pos (cond
                                      ((numberp position) position)
                           ...
                           (if (not node)
                               (format "No node found at position %s" position)
                             (format "Detailed Node Info:\n\nFile: %s (%s)\nPosition: %s\n\n=== NODE ===\nType: %s\nRange: %s\nText: %s\n\n=== PARENT ===\nType: %s\nRange: %s\n\n=== CHILDREN (%d) ===\n%s%s\n\nField names available:\n%s"
                                     filepath language position
                                     (treesit-node-type node)
                                     ...
                                     (let ((text (treesit-node-text node t)))
                                       (if (> (length text) 100)
                                         ...
                                         text))
                                     (if parent (treesit-node-type parent) "none (root)")
                                     (if parent (gptel-treesitter--node-to-range-string parent) "N/A")
                                     (length children)
                                     (if (and show-children children)
                                         (mapconcat (lambda (child)
                                                    ...
                                                    children "\n")
                                       (if children
                                           (format "  [Use show_children=true to see all %d children]" (length children))
                                         "  none"))
                                     (if (not show-children) "\n\nTip: Use show_children=true for full child list" "")
                                     (let ((fields '("name" "parameters" "body" "type" "value" "superclass")))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Get detailed information about a node including parent, children, and fields.
...
(gptel-make-tool
 ...
 :function (lambda (filepath &optional depth start-line end-line)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath))
                         ...
                         (let* ((parser (treesit-parser-create language))
                           ...
                           (cl-labels ((format-tree (node current-depth)
                             ...
                             (let ((result (format "Syntax Tree:\n\nFile: %s (%s)\nDepth: %d (max 5)\n%s\n%s"
                                                   filepath language max-depth
                                                   (if (and start-line end-line)
                                                       (format "Range: L%d-L%d\n" start-line end-line)
                                                     "Range: Full file\n")
                                                   (format-tree target-node 0))))
                               (gptel-treesitter--result-limit result))))))))
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Get AST subtree representation with configurable depth.
...
(gptel-make-tool
 :name "list_functions"
 :function (lambda (filepath &optional include-signatures limit)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath))
                                                   ...
                                                   (mapconcat
                                                    (lambda (node)
                                                      (let* ((line (line-number-at-pos (treesit-node-start node)))
                                                        ...
                                                        (format "  %s:%d: %s" (file-name-nondirectory filepath) line sig)))
                                                    functions "\n"))))
                               (gptel-treesitter--result-limit result))))))))
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Extract all function/method definitions with signatures.
...
(gptel-make-tool
 :name "list_classes"
 :function (lambda (filepath &optional include-members include-inheritance limit)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Extract all class/struct/interface declarations.
...
(gptel-make-tool
 :name "list_imports"
 :function (lambda (filepath &optional group-by-source)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath)))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Extract import/require/include statements.
...
(gptel-make-tool
 ...
 :function (lambda (filepath symbol-name)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath)))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Get complete definition of a specific symbol (function, class, etc.).
...
(gptel-make-tool
 :name "query_nodes"
 :function (lambda (filepath query &optional limit)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Execute tree-sitter query patterns (most powerful tool).
...
(gptel-make-tool
 ...
 :function (lambda (filepath node-types &optional limit)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Find all nodes of specific type(s) - simpler alternative to query_nodes.
...
(gptel-make-tool
 ...
 :function (lambda (filepath start-line end-line &optional node-type)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath)))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Find nodes within a specific line range.
...
(gptel-make-tool
 ...
 :function (lambda (filepath position &optional show-siblings)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath)))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Get surrounding context for a node (parent chain, siblings).
...
(gptel-make-tool
 :name "get_scope_structure"
 :function (lambda (filepath &optional start-line end-line)
             (condition-case err
                 (with-temp-buffer
                   (insert-file-contents filepath)
                   (let* ((language (gptel-treesitter--detect-language filepath)))
               ...
               (error (format "Error: %s\n\nTry: check_treesitter_parser(\"%s\")"
                              (error-message-string err) filepath))))
 :description "Understand nesting depth and scope hierarchy.
