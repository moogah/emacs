...
Twelve tools are scope-aware and require permission via session scope plans before
...
Two tools remain unscoped (language/parser management):
- list_treesitter_languages
- check_treesitter_parser

Permission is validated based on the filepath against glob patterns defined in
~/.gptel/sessions/SESSION_ID/scope-plan.yml.
