...
- =drawer-text= is rendered via
  ...
  =allowed-paths= + the standard deny set, and — when the preset
  declares a non-empty =:system= — =:GPTEL_SYSTEM_PROMPT_FILE:
  system-prompt.<ext>= pointing at the sibling file written by
  =jf/gptel--write-system-prompt-sibling-file=.  The basename is
...
Sibling =system-prompt.<ext>= is also written next to =session.org=
when the preset declares =:system= (design.md §Decision 1 of
=replace-system-prompt-heading-with-sibling-file=): chat-mode
re-reads it on activation and on every send so a mid-session edit
to the file is picked up without reopening the buffer.
