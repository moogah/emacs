...
The preamble is no longer a hard-coded =defconst= here.  It is the
canonical *static* leading fragment of the agent context, sourced from
=config/gptel/presets/sources/agent-preamble.org= and pre-rendered to a
committed =.txt= artifact (=register/invariant/static-prerender-dynamic-
compose=, load-bearing).  Loading that source module sets the composer's
*agent lead* seam =jf/gptel-fragment-agent-preamble-text= to the rendered
text; the writer reads that seam (the SAME pre-rendered text the per-send
composer's agent default places as the lead) so there is exactly one
preamble producer.

Composition order is preamble-first: the preset's =:system= (the
agent's *role*) is appended after the preamble so role-specific guidance
reads as a refinement of the baseline contract
(=register/invariant/context-default-composition=: agent →
=[agent-preamble(static), role, environment(dynamic)]=).  When the preset
declares no =:system=, the sibling file is the preamble alone — so every
agent now has a non-empty system prompt and a
=:GPTEL_SYSTEM_PROMPT_FILE:= drawer key (a behaviour change from the
preset-only era, where a preset without =:system= produced no sibling
file).  The dynamic environment tail is NOT materialized into the sibling
file: it is composed per send when the agent runs as a chat-mode buffer.

The agent-preamble source is required softly (=nil t=) so this module
stays load-safe when the presets sub-module has not loaded; the seam
=jf/gptel-fragment-agent-preamble-text= resolves at write time (agents are
created only after the gptel subsystem, including the presets sources, has
loaded).  The seam defaults to =""= until that source loads.
