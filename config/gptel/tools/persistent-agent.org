...
=paths.write= and =paths.deny=. Called explicitly after
=jf/gptel--create-session-core= so the agent's allowed-paths take
effect (overwriting the preset-derived scope.yml that
=create-session-core= writes).
