...
Construct the heading-and-user-block body written to =session.org=
for a fresh agent session. The output is exactly the canonical
=register/shape/session-document-layout= shape: a folded
=* System Prompt= heading (body seeded from the agent preset's
=:system= text, empty when the preset declares none), followed by a
=* Chat= heading holding the populated =#+begin_user= block (the
...
share the heading helper =jf/gptel--session-headings-block= as the
single source of truth for the heading layout. The two paths
differ only in what populates the user block:
...
Both paths therefore emit a session.org that
=shape/validate-session-document-layout= accepts (per
register/shape/session-document-layout structural invariants:
config drawer at point-min, exactly one =* System Prompt= heading,
exactly one =* Chat= heading, turn blocks under =* Chat=).
...
** Initial-Content Builder (legacy, test-only)

Pre-cycle-2 production caller. Retained for backward compatibility
with helper-level specs that pin the standalone shape (drawer +
populated user block, no =:GPTEL_SCOPE_*= keys). Production —
=jf/gptel-persistent-agent--task= — no longer routes through this
function; it composes drawer-text from
=jf/gptel-scope-profile--render-drawer-text= (which carries the
resolved =:GPTEL_SCOPE_*= keys) with the body from
=--initial-body=.
...
- =drawer-text= is rendered via
  ...
  =:GPTEL_PRESET:=, =:GPTEL_PARENT_SESSION_ID:=, and the resolved
  =:GPTEL_SCOPE_READ:= / =:WRITE:= / =:DENY:= keys built from
  =allowed-paths= + the standard deny set.
- =body= is the canonical heading block from
  =jf/gptel-persistent-agent--initial-body=: a folded
  =* System Prompt= heading (body seeded from the preset's =:system=
  text) followed by a =* Chat= heading holding the populated
  =#+begin_user= block (the agent's initial prompt).

This matches the interactive creation path's document shape
(=register/shape/session-document-layout=) so the agent
=session.org= is a conforming producer of that register entry.
