...
Construct the populated user block written to =session.org=
(Decision 4 of design.md, restructured for the
gptel-scope-in-org-properties drawer-resident scope: the drawer
itself is rendered separately by
=jf/gptel-scope-profile--render-drawer-text= and prepended at the
call site).

The body shape mirrors =jf/gptel--initial-session-body= in
=sessions/commands.el=, but with the supplied prompt embedded inside
the =#+begin_user= block. The creation-spec test asserts on the full
layout (drawer + body), so format drift fails the test.
...
- =body= is the populated user block from
  =jf/gptel-persistent-agent--initial-body=.
