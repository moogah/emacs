...
Build the agent's scope plist from =allowed-paths= plus the standard
deny set (Decision 1 of design.md, gptel-scope-in-org-properties:
...
- =drawer-text= is rendered via
  ...
  =allowed-paths= + the standard deny set, and always
