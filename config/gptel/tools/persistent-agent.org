...
- Maintain a parent-child link via a self-describing =:PROPERTIES:= drawer
  inside the agent's =session.org=.
...
Hand-format the agent's =scope.yml= behind a named function (Decision 1
of design.md). Single fixed-shape file: variable =paths.read=, constant
=paths.write= and =paths.deny=. Called by =--task= immediately after
=jf/gptel--create-agent-directory= so the agent's =scope.yml= is in
place before =find-file-noselect= opens its =session.org= and the
auto-init pipeline activates.
...
Construct the drawer + populated user block written to =session.org=
before =find-file-noselect= opens it (Decision 4 of design.md).

The format mirrors =jf/gptel--initial-session-content= in
=sessions/commands.el=, but with the supplied prompt embedded inside
the =#+begin_user= block. Cross-reference: keep shapes aligned. The
creation-spec test asserts on the full layout, so format drift in
...
overlay anchor, creates the agent directory + scope.yml + session.org,
opens the session file with =find-file-noselect= (Decision 3), and
sends via the chat-mode public API. No custom =:callback= — chat-mode's
stream callback owns text insertion.
...
is dropped — the constant deny list lives in =--write-scope-file=.
