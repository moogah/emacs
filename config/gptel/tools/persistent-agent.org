...
- =body= is the populated =#+begin_user= block from
  ...
  prompt). No headings are emitted; the preset's =:system= is
  materialised in a sibling =system-prompt.<ext>= file by a separate
  task in =replace-system-prompt-heading-with-sibling-file=.
