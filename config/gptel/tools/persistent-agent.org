...
├── sessions/              - 11 session management modules
│   ├── filesystem.el      - Directory structure, path helpers
│   ├── metadata.el        - Session metadata (model, backend, timestamps)
│   ├── registry.el        - Global session tracking
...
sequenceDiagram
    ...
    T->>FS: Write metadata.json
    T->>FS: Write preset.json
    T->>FS: Link to parent session
...
└── parent-session-20260121143022/
    ├── metadata.json                    # Parent session metadata
    ├── preset.json                      # Parent configuration
    ├── context.md                       # Parent conversation
    ├── tools.md                         # Parent tool calls
    └── subagents/                       # Subagent sessions
        └── researcher-20260121143245-analyze-code/
            ├── metadata.json            # Subagent metadata (includes parent-session-id)
            ├── preset.json              # Agent-specific config (NO inheritance)
            ├── context.md               # Subagent conversation (auto-saved)
            └── tools.md                 # Subagent tool calls
#+end_example
...
4. Check metadata.json includes =:parent-session-id=
...
8. Verify context.md contains full conversation
