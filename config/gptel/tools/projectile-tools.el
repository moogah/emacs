...
results in large projects."
 ...
 :function (lambda (directory limit filter-pattern)
             (let ((project-root (projectile-project-root directory)))
               (if (not project-root)
                   (list :success nil
...
Returns directories relative to project root, sorted alphabetically."
 ...
 :function (lambda (directory limit)
             (let ((project-root (projectile-project-root directory)))
               (if (not project-root)
                   (list :success nil
                         :error "no_project_found"
...
The search uses ripgrep, ag, or grep (fastest available) and respects .gitignore rules."
 ...
 :function (lambda (directory search-term file-pattern)
             (let ((project-root (projectile-project-root directory)))
               (if (not project-root)
                   (list :success nil
                         :error "no_project_found"
...
- Java: implementation ↔ test (Foo.java ↔ FooTest.java)"
 ...
 :function (lambda (directory file-path relationship-type)
             (let ((project-root (projectile-project-root directory)))
               (if (not project-root)
                   (list :success nil
                         :error "no_project_found"
