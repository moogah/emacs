...
Built on gptel's preset system (=gptel--known-presets=) for robust, configurable agent execution.
...
*Standard Tool*: Use the =PersistentAgent= tool to invoke agents via presets
...
When presets are registered, =@mentions= are automatically expanded with skill content via =jf/gptel-preset--expand-all-agent-skills=.

*** Register New Preset

After creating the preset file in =config/gptel/presets/=, register it:

#+begin_src emacs-lisp
(jf/gptel-preset-register-all)  ; Scans preset directory and registers all presets
#+end_src

The preset will now appear in =gptel--known-presets= and be available for sessions.

** Configuration

*** Preset Directory

Presets are stored in =config/gptel/presets/= as =.md= files with YAML frontmatter.
Registration is handled by =jf/gptel-preset-register-all= in =gptel.org=.
...
Current preset-based implementation:
- Invisible background execution
- File-based preset definitions (=config/gptel/presets/*.md=)
- =PersistentAgent= tool for persistent sessions
- Proven FSM and state management
- Skills integration support
- Preset registration via =gptel--known-presets=
...
- =community-tools.org= :: External tool packages (ragmacs, etc.)
...
- =subagent-tools.org= :: Legacy agent implementation (superseded by preset-based system)
...
- [[https://github.com/karthink/gptel][gptel]] - LLM client with preset and tool support
