...
in =gptel/presets/registration.org=. Presets are authored as =.org= sources
tangling to =config/gptel/presets/<name>/preset.el= artifacts that self-register
via =jf/gptel-preset-register=; the loader =jf/gptel-preset-register-all= loads them.
