...
** Phase 4: Text Input Questions

Goal: Support freeform text input questions.

Phase 4 adds text input support:
- New suffix type that prompts for text using `read-string`
- Support for custom prompt text
- Support for default values
- Works with mixed question types (choice + text)

Key changes:
1. Added `jf/gptel-ask--build-text-suffix` to generate text input suffix
2. Added `jf/gptel-ask--make-text-handler` using `read-string`
3. Modified `jf/gptel-ask--build-suffixes` to handle "text" type
4. Text answers recorded without key (nil for key field)

*** Testing Phase 4

**** Test 1: Single text question
...
Expected behavior:
1. Menu displays: "Question 1/1: Preferred library?"
2. Single option: "Enter Library" (press RET)
3. Minibuffer prompts: "Library name: " with "react" as default
4. User enters text (or accepts default)
5. Menu closes, result JSON returned with text answer

**** Test 2: Mixed questions (choice + text)
  ...
  "questions": [
    {
      ...
      "type": "choice",
      "options": [
        {"key": "a", "label": "React", "description": "Facebook's UI library"},
...
Expected behavior:
1. First question: "Question 1/2: Which framework?" (choice)
2. User selects option (e.g., 'a' for React)
3. Second question: "Question 2/2: Project name?" (text)
4. User presses RET, enters project name
5. Menu closes, result JSON with both answers returned

**** Test 3: Text without default
...
Expected behavior:
- Prompt defaults to "Username: " (from header field)
- No default value in minibuffer
- User types username and presses RET

** Phase 5: Error Handling and Polish

Goal: Production-ready robustness with proper cancellation handling.

Phase 5 adds error handling for user cancellation:
- Track active question flow with global variable
- Use transitioning flag to distinguish question transitions from cancellations
- Detect C-g cancellation via transient-exit-hook
- Return "cancelled" status to LLM when user quits
- Clean up state on cancellation

Key changes:
1. Added `jf/gptel-ask--active-callback` to track active flow
2. Added `jf/gptel-ask--transitioning` flag to prevent false positives
3. Added `jf/gptel-ask--handle-exit` hook function
4. Registered hook on `transient-exit-hook`
5. Modified handlers to set transitioning flag during question transitions
6. Use `run-at-time` with 50ms delay to clear transitioning flag (exit hook runs asynchronously)
7. Modified `jf/gptel-ask--finish` to clear all state (prevent double-invocation)
8. Modified `jf/gptel-ask-user-questions` to initialize state on start

*** Why the Timer?

The exit hook runs asynchronously after `transient-setup` returns. If we clear the
transitioning flag immediately, the hook will see it as nil and incorrectly invoke
the callback. By using a 50ms timer, we ensure the flag stays set long enough for
the exit hook to check it.

*** How Cancellation Works

1. **Normal completion path:**
   - User answers all questions
   - `jf/gptel-ask--finish` is called
   - Clears `jf/gptel-ask--active-callback` and `jf/gptel-ask--transitioning`
   - Calls callback with "completed" status
   - Exit hook sees nil callback, does nothing

2. **Question transition path:**
   - User answers a question (not the last one)
   - Handler sets `jf/gptel-ask--transitioning` to t
   - Calls `transient-setup` for next question (returns immediately)
   - Schedules timer to clear `jf/gptel-ask--transitioning` after 50ms
   - Exit hook runs (asynchronously), sees transitioning = t, does nothing
   - Timer fires later, clears transitioning flag

3. **Cancellation path (C-g):**
   - User presses C-g in transient menu
   - Transient exits without calling any suffix
   - `transient-exit-hook` runs
   - `jf/gptel-ask--handle-exit` checks: active callback? yes, transitioning? no
   - Calls callback with "cancelled" status
   - Clears callback and state

*** Testing Phase 5

**** Test 1: Normal completion (no change expected)

Ask LLM to use tool with any question format. Answer all questions normally.
Expected: Same behavior as Phase 4, callback receives "completed" status.

**** Test 2: Cancel after first question (multi-question flow)

Ask LLM to use tool with 2-3 questions. Answer first question, then press C-g
on second question.

Expected:
- Menu closes immediately
- LLM receives JSON: `{"status":"cancelled","answers":[]}`
- gptel conversation continues (FSM doesn't hang)
- LLM acknowledges cancellation in response

**** Test 3: Cancel before answering any questions

Ask LLM to use tool. Press C-g immediately when menu opens.

Expected:
- Menu closes
- LLM receives cancellation status
- No answers in result array
- Conversation continues normally

**** Test 4: Concurrent sessions (needs further testing)

**STATUS:** Not tested yet. Known limitation with global variables.

Test scenario: Open two gptel buffers. Ask both to use the tool. Interact with
questions in different order (answer buffer 2, then buffer 1).

Expected behavior (ideal):
- Each session's callback goes to correct session
- No interference between sessions
- Cancel in one session doesn't affect the other

Current limitation:
- `jf/gptel-ask--active-callback` and `jf/gptel-ask--transitioning` are global
- Second call will overwrite first call's cancellation tracking
- Questions/answers are isolated via :scope (works correctly)
- Callbacks are isolated via :scope (works correctly)
- Cancellation tracking is NOT isolated (may cause issues)

Recommendation: Avoid concurrent use until this is fixed with buffer-local variables
or a session registry.

*** Additional Polish Items

Items already implemented in previous phases:

✅ **Progress indicator:** Shows "Question N/M" (Phase 3)
✅ **Clear error messages:** JSON validation with detailed errors (Phase 1)
✅ **Descriptive section headers:** Question text shown in menu description
✅ **Mixed question types:** Choice and text work together seamlessly (Phase 4)
✅ **C-g cancellation:** Returns "cancelled" status to LLM (Phase 5)

Items considered but not needed:

- **Key binding conflict validation:** Not needed - LLM-provided keys are single chars,
  transient handles conflicts automatically. Reserved keys (C-g, C-h) work as expected.

- **Logging for debugging:** Not needed for production use. Error messages from
  validation are sufficient. Can be added later if issues arise.

Known limitations:

⚠️ **Concurrent session support:** Questions/answers are isolated via :scope (works),
  but cancellation tracking uses global variables (limitation). Second concurrent call
  will overwrite first call's cancellation tracking. Avoid concurrent use until fixed
  with buffer-local variables or session registry. Needs further testing.
