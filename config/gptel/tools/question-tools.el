...
(defun jf/gptel-questions--add-comment (question-id)
  "Prompt for and add comment to QUESTION-ID."
  (interactive)
...
(defun jf/gptel-questions--format-question (question)
  "Format QUESTION for display in transient menu."
...
(cl-defmethod transient-infix-set ((obj jf/gptel-question-infix) value)
  ...
  ;; Prompt for optional comment after answering
  (when (y-or-n-p "Add comment? ")
    (let ((q-id (plist-get (oref obj question) :id)))
      (jf/gptel-questions--add-comment q-id))))

(cl-defmethod transient-format ((obj jf/gptel-question-infix))
  "Format question and answer for display."
  (let* ((q (oref obj question))
         (formatted (jf/gptel-questions--format-question q)))
    (format "  %s" formatted)))
...
(defun jf/gptel-questions--setup-question-suffixes (_)
  ...
  (let* ((scope (transient-scope))
         (questions (plist-get scope :questions))
         (key-pool (append (number-sequence ?1 ?9)
                          (number-sequence ?a ?z))))

    (transient-parse-suffixes
     'jf/gptel-questions-menu
     (cl-loop for q in questions
              for key = (pop key-pool)
              when key
              collect
              (list (key-description (char-to-string key))
