...
Helpers for behavioral testing of ~jf/gptel-persistent-agent--task~.  The
agent creates a session from within a parent session buffer, so tests need
to simulate that parent context and mock the gptel upstream boundary.

** ~with-parent-session~

Sets up a temporary buffer simulating a parent persistent session.  The
buffer has the expected buffer-local session variables and can be used as
the ~current-buffer~ when calling persistent-agent code.

Cleans up the buffer and registry entry on exit.
...
Mocks the upstream gptel functions that cross the package boundary:
~gptel-get-preset~, ~gptel--apply-preset~, ~gptel-mode~, ~gptel-request~,
and ~markdown-mode~.

Each mock records its calls for assertion.  Binds tracking variables
within BODY.
