...
If BUFFER is nil, uses current buffer."
  (with-current-buffer (or buffer (current-buffer))
    (let ((mentions '())
      ...
      ;; `save-match-data' is required: this runs from
      ;; `after-change-functions', so any `re-search-forward' here would
      ;; otherwise clobber match-data of the caller mid-edit. The
      ;; concrete failure mode is `org-entry-put''s UPDATE branch reading
      ;; stale match-data from `(goto-char (match-beginning 0))' after
      ;; `delete-region' fired this hook, causing property re-inserts to
      ;; land at the position of the last `@mention' in the buffer.
      (save-match-data
        (save-excursion
          (goto-char (point-min))
