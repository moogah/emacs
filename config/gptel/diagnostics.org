...
*Upstream fix needed in gptel.el:* =gptel--restore-props= should temporarily disable
=after-change-functions= during restoration:
...
*Status:* Fix has been applied to local gptel installation at
=runtime/straight/repos/gptel/gptel.el:597=. The patch adds =inhibit-modification-hooks=
binding to prevent =after-change-functions= from running during restoration.
