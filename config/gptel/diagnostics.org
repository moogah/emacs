...
Original hypothesis: The tool property bound starts at the wrong buffer position (38 instead of 81),
causing =gptel--parse-buffer= to read from blank space instead of the tool call sexp.

*Status:* This hypothesis was partially correct about the symptom (wrong position) but the actual
cause was the diagnostic module itself accumulating advice layers (see Findings below).

** Findings

Diagnostics revealed that:
- Saved bounds in file Local Variables were correct: =tool= at position 80, =ignore= at 37-80
- After restoration, position 37 had =tool= property instead of =ignore= property
- Multiple =add-text-properties= calls during restoration, including erroneous =(37 80 tool)= call
- *Root cause:* Diagnostics module itself was accumulating advice on module reload
- Each reload added another advice layer, causing =gptel--restore-props= to apply properties multiple times
- The third/fourth application overwrote the =ignore= property at 37-80 with =tool= property

*Fix:* Ensure =jf/gptel--start-diagnostic-session= removes all existing advice before installing new advice.

*Next step:* Test with fresh Emacs restart to determine if there's an underlying gptel bug or if
the issue was entirely caused by diagnostic advice accumulation.
