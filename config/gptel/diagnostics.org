...
Load this file and diagnostics will automatically start:
=(load-file "config/gptel/diagnostics.el")=

You should see: ="GPTEL diagnostics auto-enabled - diagnostics are now active"=

The diagnostic session is now active and will capture all bounds changes and parse attempts.
