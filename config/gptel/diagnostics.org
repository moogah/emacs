...
The tool property bound starts at the wrong buffer position (38 instead of 81).
When =gptel--parse-buffer= tries =(read (current-buffer))= at position 38,
it reads from blank space instead of the tool call sexp, resulting in empty data.
...
Fixed by ensuring =jf/gptel--start-diagnostic-session= removes all existing advice before installing new advice.
