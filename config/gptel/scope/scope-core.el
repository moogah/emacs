...
(defun jf/gptel-scope--find-session-dir (session-id)
  "Find session directory for SESSION-ID.
First tries registry (in-memory), then falls back to disk lookup.

SESSION-ID can be either:
- Registry ID (short format): '20260119114629-aaed'
- Directory name (full format): '20260119-114629-claude-opus-4-5-20251101'

The function tries both formats to handle all cases."
  (or
   ;; Try registry first (fast, works for current session)
   (when-let ((session-data (jf/gptel--get-session-data session-id)))
     (plist-get session-data :directory))

   ;; Try direct directory lookup (session-id might BE the dirname)
   (let ((dir-path (expand-file-name session-id jf/gptel-sessions-directory)))
     (when (file-directory-p dir-path)
       dir-path))

   ;; Fall back to searching all session directories
   ;; Needed when session-id is registry ID but registry is empty (after restart)
       ...
       (dolist (dirname (directory-files sessions-base nil "^[0-9]\\{8\\}-[0-9]\\{6\\}-"))
         (unless found-dir
           (let* ((dir-path (expand-file-name dirname sessions-base))
                  (metadata-file (expand-file-name "metadata.json" dir-path)))
             (when (file-exists-p metadata-file)
               (condition-case nil
                   (let* ((json-object-type 'plist)
                          (json-array-type 'vector)
                          (json-key-type 'keyword)
                          (metadata (json-read-file metadata-file))
                          (meta-session-id (plist-get metadata :session_id)))
                     ...
                     (when (or (string= meta-session-id session-id)
                               ;; If session-id is registry format, convert to directory prefix
                               (and (string-match "^\\([0-9]\\{8\\}\\)\\([0-9]\\{6\\}\\)-[0-9a-f]\\{4\\}$" session-id)
                                    (let ((dir-prefix (concat (match-string 1 session-id) "-" (match-string 2 session-id))))
                                      (string-prefix-p dir-prefix meta-session-id))))
                       (setq found-dir dir-path)))
                 (error nil)))))))
     found-dir)))
...
Returns parsed plan as plist or nil if not found."
  (condition-case err
      (when-let ((session-dir (jf/gptel-scope--find-session-dir session-id)))
...
;; [[file:scope-core.org::*Check Filesystem Write Scope][Check Filesystem Write Scope:1]]
...
;; Check Filesystem Write Scope:1 ends here
...
;; [[file:scope-core.org::*Check Org-Roam Write Scope][Check Org-Roam Write Scope:1]]
...
Returns plist with :allowed t/nil and relevant patterns."
  (let* ((roam-data (plist-get plan :org_roam))
    ...
    (if allowed
      ...
      (list :allowed nil
            :allowed_patterns write-patterns))))
;; Check Org-Roam Write Scope:1 ends here
...
;; [[file:scope-core.org::*Check Shell Command Scope][Check Shell Command Scope:1]]
...
;; Check Shell Command Scope:1 ends here
...
;; [[file:scope-core.org::*Format Scope Violation Error][Format Scope Violation Error:1]]
  ...
  (let ((allowed-patterns (plist-get check-result :allowed_patterns))
    ...
    (list :success nil
          ...
          :message (format "%s operation to '%s' denied by scope plan. Use request_scope_expansion tool to ask user for approval."
                          ...
                          resource))))
;; Format Scope Violation Error:1 ends here
