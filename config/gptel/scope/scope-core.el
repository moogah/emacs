          ...
          (cl-loop for (key value) on parsed by #'cddr
                   do (let ((key-name (substring (symbol-name key) 1)))
                        ...
                        (cond
                         ...
                         ;; Handle tools section (map of tool names to empty plists)
                         ((eq key :tools)
                          (insert (format "%s:\n" key-name))
                          ;; tools is a plist like (:read_file nil :write_file_in_scope nil ...)
                          (cl-loop for (tool-key _tool-props) on value by #'cddr
