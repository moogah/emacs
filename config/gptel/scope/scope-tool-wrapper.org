...
The canonical tool-result serializer (=jf/gptel-scope--serialize-tool-result=)
and its sanitizer now live in =scope-validation=, the common ancestor every
scope module requires, so allow/deny/allow-once/add-to-scope and the bare-tool
acknowledgements all funnel through ONE serializer that enforces the JSON
contract (array fields are JSON arrays; string bytes are UTF-8 safe). The macro
below calls it; it is resolved from =scope-validation= (required above). See
that module's "Tool Result Serialization" section.
