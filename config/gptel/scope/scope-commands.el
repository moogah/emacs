...
;; Helper function to generate tool definition with worktree patterns.
...
"
          ...
          (if worktree-patterns
            ...
            "      []\n")))
;; Tool Definition Helper:1 ends here
...
Note: run_approved_command is skipped (not yet implemented)."
  ...
  (let* ((timestamp (format-time-string "%Y-%m-%dT%H:%M:%SZ"))
         ...
         ;; Format worktree paths for patterns
         ;; Note: worktree-paths should already have /** suffix (full glob patterns)
