...
This module provides scope-aware shell execution and meta tools for v2.0 tool-level permission management:

1. *Shell tool*: =run_approved_command= - Execute shell commands with pattern checking
2. *Meta tool*: =request_scope_expansion= - LLM requests user approval to expand specific tool permissions
3. *Meta tool*: =inspect_scope_plan= - LLM inspects current scope plan to understand existing permissions before requesting expansion

** Shell Command Scoping (v2.0)

The =run_approved_command= tool uses tool-level configuration:
- *patterns*: Whitelist of allowed command names (e.g., ["ls", "git", "npm"])
- *deny_patterns*: Blacklist patterns (substring matching, e.g., ["rm -rf", "sudo"])

Deny patterns override allow patterns for security.

** Scope Expansion (v2.0)

The =request_scope_expansion= tool is now tool-specific (not category-based):
- LLM specifies exact tool name: "write_file_in_scope", "edit_file_in_scope", etc.
- Patterns added only to that specific tool
- Provides fine-grained control (can grant write but deny edit)
