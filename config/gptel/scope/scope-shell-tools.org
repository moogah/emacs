...
The =run_approved_command= tool uses scope.yml configuration:
...
In v3.0, scope.yml is a plain YAML file.
LLM can use read_file("scope.yml") to inspect scope configuration directly.
