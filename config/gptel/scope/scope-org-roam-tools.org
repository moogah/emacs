#+title: GPTEL Scope Manager - Org-Roam Tools
#+author: Jeff Farr
#+property: header-args:elisp :tangle scope-org-roam-tools.el :comments both

* Overview

This module provides scope-aware org-roam tools for gptel sessions. Tools check approved scope patterns for org-roam operations (node creation, linking, tag management) and return structured errors when operations are outside approved scope.

** Org-Roam Scope Patterns

Scope plans support two pattern types:

1. *Subdirectory patterns*: =subdirectory:gptel/**= - Controls where nodes can be created
2. *Tag patterns*: =tag:gptel= - Controls which tags can be added to nodes
3. *Link patterns*: =node_id:*= or =node_id:SPECIFIC-ID= - Controls which nodes can be linked
