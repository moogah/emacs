...
Most templates grant read_file permissive =/**= access to any file (except .env, .git, runtime). The project-aware template restricts reads to selected project directories.
