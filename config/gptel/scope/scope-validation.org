...
1. *Parse completeness* — bail out if bash-parser couldn't fully parse
   the command and =security.enforce-parse-complete= is on.
...
The drawer reader is the new producer of =register/shape/scope-config-
plist=. It implements =register/boundary/scope-config-loader= as a
two-stage pipeline: stage 1 reads the file-level =:PROPERTIES:= drawer
of a *live chat buffer* (the source of truth for in-flight edits);
stage 2 falls back to opening =session.org= headlessly when no buffer
...
=:cloud=. =:security= is forbidden by =register/invariant/scope-no-
security-key-in-plist= and is not produced here.

This task lands the functions defined-but-not-wired-in. The validator
still loads via the YAML path above; =rewire-validator-config-load=
redirects =jf/gptel-scope--load-config= to call into
=jf/gptel-scope--load-config-from-drawer= (or renames it) once the
=:security= consumer-side reads have been replaced by the constants
from =* Constants=.
