...
about expansion UI or error formatting.
...
=jf/gptel-scope--load-config= always returns a plist (deny-all
defaults when no drawer / empty drawer); the dispatcher therefore
proceeds straight to validation. Per-violation deny is the
authorisation outcome for an empty drawer — the historical
=no_scope_config= short-circuit is gone (Option B; see
=register/boundary/scope-config-loader=). =--final-deny-response=
formats every denial via =format-tool-error= for LLM consumption.
