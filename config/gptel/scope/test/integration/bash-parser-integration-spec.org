...
*** Scenario 7: Delete Out of Scope (=rm /etc/passwd=)

Tests path-based denial for delete operations. =/etc/passwd= matches the
workspace scope's deny pattern, so the =:delete= operation produces a
=denied-pattern= error.
