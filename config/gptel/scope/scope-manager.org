#+TITLE: GPTEL Tool Scope Manager
#+PROPERTY: header-args:elisp :tangle scope-manager.el :lexical t

* Overview

**STATUS: This module implements an ALTERNATE approach (advice-based interception) that is NOT actively used in v2.0.**

The preferred v2.0 approach uses tool-internal validation via =gptel-make-scoped-tool= macro (see scope-*-tools.el files). This alternate implementation is preserved for reference and potential future use.

This scope manager enforces fine-grained access control for gptel tools using glob-pattern-based allowlists. When LLM agents attempt to modify resources (files, org-roam nodes, shell commands) outside approved scope, the system intercepts execution and prompts for user approval.

** Design Principles (v1.0 - Legacy)

- *Deny by default*: Secure by default, requires explicit approval
- *Write-only scoping*: Read operations always allowed, only writes require approval (NOTE: v2.0 uses scope-controlled reads)
- *Defense in depth*: Works alongside gptel's existing confirmation system
- *Session-based*: Each session has independent scope plan

** Architecture (v1.0 Format)

This implementation uses v1.0 YAML format with category-based permissions (filesystem, org_roam, shell) rather than v2.0 tool-level format.

Scope plans are stored as YAML files. The path structure depends on session implementation (may be ~~/gptel-sessions/SESSION_ID/scope-plan.yml~ or in branch directories).

The system intercepts tool execution via advice on ~gptel--accept-tool-calls~ (after confirmation UI, before tool execution).
...
Convert glob pattern to regex for matching file paths.

Glob syntax:
- ~**~ matches any path component (including /)
- ~*~ matches any characters except /
- ~?~ matches single character except /
- Literal braces, dots, etc. are escaped
...
Helper to check against multiple patterns.
...
** YAML Plan Management

*** Convert Vectors to Lists

YAML parser returns arrays as vectors, but our code expects lists.
...
Load and parse YAML plan from session directory.
...
Write updated plan to YAML file.
...
Classify tools as read or write operations. Only write operations require scope checking.
...
*** Check Resource Against Plan

Main validation logic. Checks if a resource is allowed by the plan.
...
Extract resource identifiers from tool arguments. Returns list of (type operation resource-path) tuples.
...
Conservative validation for shell commands. Uses whitelist approach.
...
** Check Tool Calls (Advice Function)

Main advice function that intercepts tool execution and enforces scope checking.
...
** Prompt for Violation Approval

Display violations and prompt user for approval.
...
** Add Violations to Plan

Extract patterns from violations and add to plan file by directly editing YAML text.
...
Secure default - nothing allowed, user must add patterns.
...
Allow reading project files, deny writes by default.
...
Allow org-roam operations in gptel subdirectory only.
...
Manually test tool against plan (debugging).
...
Install scope checking advice after gptel loads.
