...
The scope manager enforces fine-grained access control for gptel tools using glob-pattern-based allowlists. When LLM agents attempt to modify resources (files, org-roam nodes, shell commands) outside approved scope, the system intercepts execution and prompts for user approval.

** Design Principles

- *Deny by default*: Secure by default, requires explicit approval
- *Write-only scoping*: Read operations always allowed, only writes require approval
...
Scope plans are stored as YAML files in ~~/gptel-sessions/SESSION_ID/scope-plan.yml~.
