...
**Primary:** =scope.yml= (plain YAML, no frontmatter) in the same directory as =session.md=

**Legacy fallback:** =preset.md= (YAML frontmatter between =---= delimiters) for backward compatibility.
...
use this to find =scope.yml= (or legacy =preset.md=) in the same directory as the context file.
