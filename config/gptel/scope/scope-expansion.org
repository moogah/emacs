...
=:operation= keyword to one of five list-shaped drawer keys
(=GPTEL_SCOPE_READ= / =WRITE= / =MODIFY= / =EXECUTE= / =DENY=). The bare
form is returned (no surrounding colons) because Org's =org-entry-*=
property API takes the bare key; the colonised =:GPTEL_SCOPE_READ:= literal
appears only in the drawer text.
...
Fail-safe default: =nil= operation maps to =GPTEL_SCOPE_READ= (the
most-restrictive scope). A missing-operation bug surfaces in the dedup loop
rather than in over-permissive grants.
