...
command-name resources (bare names like "tree"), the function returns
the sentinel =:bare-command-refusal= — command-level expansion is not
supported in the operation-first model (commands are validated by the file
operations they perform, not by name).

The sentinel is distinct from =nil= (which signals dedup short-circuit)
so the OUTER action handler (=--add-to-scope= and friends) can emit a
structured denial payload to the wrapper's callback (=scope-rearch-
followups= Bug 2 fix). Before the fix, the bare-command branch and dedup
branch both returned =nil= and the wrapper received the dedup payload
(=success: t patterns_added: []=) — the LLM interpreted that as "added,
retry", retried, hit the same denial, and looped.
