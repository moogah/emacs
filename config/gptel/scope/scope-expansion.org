...
resources to =--write-pattern-to-drawer=. For command-name resources
(bare names like "tree"), the function emits a user message and invokes
the callback with =:success nil= — command-level expansion is not
supported in the operation-first model (commands are validated by the
file operations they perform, not by name). The callback invocation on
the bare-command branch is the =scope-rearch-followups= Bug 2 fix folded
into this rewire (per Decision 7) so the wrapper always receives a
structured response.
