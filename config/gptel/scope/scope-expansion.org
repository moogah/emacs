...
Encapsulates the =pcase= routing block so it can be reused by wildcard and custom suffix commands.
...
=:operation= keyword to one of four list-shaped drawer keys (=READ= /
=WRITE= / =MODIFY= / =EXECUTE=). The bare form is returned (no surrounding
colons) because Org's =org-entry-*= property API takes the bare key; the
colonised =:GPTEL_SCOPE_READ:= literal appears only in the drawer text.
...
On read, =scope-yaml= normalizes these to kebab-case (e.g., =:org-roam-patterns=).
When writing back, these updaters use the original =snake_case= format so that YAML
files remain consistent and human-readable.
...
This function handles bash scope expansion by delegating path resources to
=add-path-to-scope=.  For command-name resources (bare names like "tree"),
the function logs a message since command-level expansion is not supported
in v4 (the operation-first model validates by file paths, not command names).
...
*** Write Generic Nested Plist

Write a generic nested plist structure (like :cloud or :security).
This handles arbitrary plists with simple values (strings, numbers, symbols, lists).
