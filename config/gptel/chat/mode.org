...
- =gptel-chat--indent-inserted-region= runs from
  ...
  *records* the inserted region and schedules an idle timer.
- =gptel-chat--indent-pending-regions= runs from that timer, outside
  any =after-change= dispatch, and performs the actual shift via
  =gptel-chat--indent-region=.
...
So the recorder records, and the shift happens later from an idle
timer, when no dispatch is active. The display layer defers its
