...
equal to the length of the *replaced* text — zero for a pure
insertion, positive when the change replaced existing content. The
gate is =(> end beg)=: we process every event whose post-change
region is non-empty. Pure deletions (=END = BEG=) introduce no new
content, so they short-circuit out; insertions and replacements both
introduce content into =[BEG, END)= and flow through.

The replacement case is load-bearing. Replacements driven by
=replace-match=, =replace-region-contents=, =query-replace=, or any
other primitive that fires =after-change-functions= with =LENGTH > 0=
AND =END > BEG= can introduce a brand-new column-0 =*= line where
none existed before — e.g., =query-replace foo → * H1= run inside a
chat-block body. Gating on =(zerop length)= would silently allow that
column-0 =*= to corrupt the surrounding =#+begin_user= /
=#+begin_assistant= / =#+begin_tool= block, violating
=register/invariant/chat-block-body-no-column-zero-stars=. The streaming
sanitizer covers the chunk-by-chunk insert path; this hook is the
catch-all for every other content-introducing change.
