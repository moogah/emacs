...
- Outer delimiter lines (=#+begin_user=, =#+end_assistant=, etc.) are
  NOT covered by role overlays. They stay in the default face so users
  can still see block structure at a glance.
- Nested =#+begin_tool= / =#+end_tool= delimiter lines stay at real
  column 0 (the parser anchors them there — =gptel-chat-heading-scoping=
  design.md Decision 5, Path C) but are rendered with a =line-prefix=
  so they align visually with the indented body around them. The
  prefix is display-only; buffer text is unchanged.
