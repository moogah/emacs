...
(defun gptel-chat--fold-config-drawer ()
  "Fold the file-level config `:PROPERTIES:' drawer in the current buffer.

Runs from `gptel-chat-mode-hook' on activation.  Folds every drawer
via `org-fold-hide-drawer-all', which covers the pre-first-heading
file-level configuration drawer that org-mode's startup visibility
does not reliably reach in this derived mode.
...
;; are minimal by design: the keymap (above) and =org-adapt-indentation=
;; (in the body); the config-drawer fold runs from
;; =gptel-chat-mode-hook= (=gptel-chat--fold-config-drawer=, attached
;; after the form below).
