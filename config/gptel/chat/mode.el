...
`org-fold-hide-drawer-all' — a buffer-wide fold, not a targeted
one.  Covers the pre-first-heading file-level configuration
drawer that org-mode's startup visibility does not reliably
reach in this derived mode.
...
A no-op for buffers without a drawer: a `gptel-chat-new' scratch
buffer (no drawer, no headings) raises no error.  See design.md
