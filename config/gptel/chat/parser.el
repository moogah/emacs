...
(defconst gptel-chat--re-block-delimiter
  "^#\\+\\(begin\\|end\\)_\\(user\\|assistant\\|tool\\)\\b"
  "Union regex matching ANY chat-block delimiter line at column 0.
Group 1 is the side: `begin' or `end'.  Group 2 is the kind: `user',
`assistant', or `tool'.  Used by the point-in-block-body predicate to
walk delimiter lines without re-parsing the whole buffer.")
...
;; - POS strictly inside the body of any =#+begin_user=,
;;   =#+begin_assistant=, or =#+begin_tool= block (i.e., past the end of
;;   the opener line and before the start of the matching closer line)
;;   → return non-nil.
...
;;   a block body for this predicate's purposes — a =*= heading inside
;;   a tool body would corrupt the surrounding assistant block just as
;;   surely as one in the assistant text proper.

;; Algorithm: walk backward from POS through delimiter lines maintaining
;; a closer-stack.  Each =#+end_*= encountered pushes onto the stack;
;; each =#+begin_*= pops if the stack is non-empty, otherwise it is the
;; "enclosing" opener and we stop.  If we reach BOB without finding an
;; unmatched opener, POS is not inside any block.  Then forward-scan
;; for the matching closer using a depth counter for nested same-kind
;; openers.
...
(defun gptel-chat--point-in-block-body-p (&optional pos buffer)
  "Return non-nil when POS is strictly inside a chat-block body.

POS defaults to `(point)'; BUFFER defaults to the current buffer.

\"Strictly inside\" means POS lies past the end-of-line of the
nearest enclosing `#+begin_user', `#+begin_assistant', or
...
Returns nil for any of:
  ...
  - POS between blocks (after a closer, before the next opener).
      ...
      (save-excursion
        (save-match-data
          (let ((case-fold-search t)
                (orig-line-bol (save-excursion
                                 (goto-char target-pos)
                                 (line-beginning-position)))
                opener-pos
                opener-kind
                (closer-stack 0))
            ;; Reject immediately when the line containing POS is
            ;; itself a delimiter line.  We use beginning-of-line
            ;; matching to avoid distinguishing column 0 vs. mid-line
            ;; for the caller.
            (goto-char orig-line-bol)
            (if (looking-at gptel-chat--re-block-delimiter)
                nil
              ;; Walk backward through delimiter lines.
              (goto-char target-pos)
              (catch 'found
                (while (re-search-backward gptel-chat--re-block-delimiter
                                           nil t)
                  (let ((side (match-string 1))
                        (kind (match-string 2)))
                    (cond
                     ((string-equal (downcase side) "end")
                      (setq closer-stack (1+ closer-stack)))
                     ((zerop closer-stack)
                      (setq opener-pos  (match-beginning 0)
                            opener-kind (downcase kind))
                      (throw 'found t))
                     (t
                      (setq closer-stack (1- closer-stack)))))))
              (when opener-pos
                ;; Verify POS is past the end of the opener's line.
                (let ((opener-eol (save-excursion
                                    (goto-char opener-pos)
                                    (line-end-position))))
                  (when (> target-pos opener-eol)
                    ;; Forward-scan for the matching closer of
                    ;; OPENER-KIND, ignoring deeper same-kind nesting.
                    (goto-char (1+ opener-eol))
                    (let ((same-kind-depth 1)
                          closer-bol)
                      (catch 'closed
                        (while (re-search-forward
                                gptel-chat--re-block-delimiter nil t)
                          (let ((side (downcase (match-string 1)))
                                (kind (downcase (match-string 2))))
                            (when (string-equal kind opener-kind)
                              (cond
                               ((string-equal side "begin")
                                (setq same-kind-depth
                                      (1+ same-kind-depth)))
                               (t
                                (setq same-kind-depth
                                      (1- same-kind-depth))
                                (when (zerop same-kind-depth)
                                  (setq closer-bol
                                        (match-beginning 0))
                                  (throw 'closed t))))))))
                      (when (and closer-bol
                                 (< target-pos closer-bol))
                        t))))))))))))
;; Point-in-block-body predicate:1 ends here
