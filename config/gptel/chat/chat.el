...
;; 3. =tool-confirm= defines the overlay UI invoked by =stream='s
;;    =tool-call= arm. Loaded BEFORE =stream= so the callback's
;;    =fboundp= guard finds =gptel-chat--display-tool-confirm= during
;;    normal init (the guard preserves test isolation, but production
;;    init should hit the real symbol). No load-time references back
;;    to =stream= or any other chat-mode module.
;; 4. =stream= builds the streaming callback; its internal
...
;;    =parser= helpers only at call time, and references
;;    =gptel-chat--display-tool-confirm= (from =tool-confirm=) only at
;;    tool-call dispatch time.
;; 5. =send= is the interactive command bound to =C-c C-c=. It calls
;;    into =parser= and =stream= from inside the command body
;;    (call-time).
;; 6. =nav= ordering after =send= is *convention*, not a load-time
...
;;    modules.
;; 8. =menu= ordering after =send= is also convention. The transient
...
(jf/load-module (expand-file-name "config/gptel/chat/tool-confirm.el" jf/emacs-dir))
