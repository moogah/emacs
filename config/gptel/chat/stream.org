...
=gptel-chat--make-stream-closure= returns a =(lambda (chunk))= that
...
- =holdback= — string carrying trailing partial-line content between
  chunks. The sanitizer only runs on complete lines; without a
  holdback, a =#+end_assistant= split across a chunk boundary would
  be inserted as two fragments neither of which matches the regex.
- =tool-marker= — Emacs marker (or nil). When non-nil, inserts go
  there instead of =insertion-marker=. This slot is exposed now so
  the later =stream-callback= task can set it on =tool-call= events
  and clear it on =tool-result=. Behaviour here: if =tool-marker= is
  a live marker, inserts go to it; otherwise they go to
  =insertion-marker=.

The returned closure accepts one argument:
