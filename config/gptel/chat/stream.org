...
| =(tool-call . ,calls)=         | Open one =#+begin_tool (<name> :args <sexp>)= block per call; route subsequent inserts into the LAST opened block via =set-tool-marker=; push each call's result-target marker onto a FIFO. |
