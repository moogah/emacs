...
(defun gptel-chat--stream-format-tool-header (name args)
  "Format a `#+begin_tool' header line for tool CALL plist.
NAME is a string.  ARGS is a plist-or-list (model-supplied tool
arguments) rendered via `prin1-to-string' so the parser's
`read-from-string' round-trips it intact.  Returns the full
...
(defun gptel-chat--stream-open-tool-block (insertion-marker call)
  "Open a nested `#+begin_tool' block at INSERTION-MARKER for CALL.
CALL is a plist carrying at least `:name' (string) and `:args'
\(plist or list\).  Inserts both `#+begin_tool ...' and matching
`#+end_tool' delimiters with an empty body, and returns an advance
marker (insertion-type t) positioned on the empty body line
between them.  The caller feeds the result text into that marker
via `gptel-chat--stream-close-tool-block'; the closing delimiter
is already in place.
...
RESULT is a string (or nil for an empty result).  Each line of
RESULT is sanitized via `gptel-chat--sanitize-chunk' so a result
that itself contains `#+end_tool' or `#+end_assistant' cannot
prematurely close a containing block."
  (gptel-chat--stream-insert-sanitized-block tool-marker
                                             (or result "")))
...
- PENDING-TOOL-MARKERS: FIFO list of result-target markers, one
  ...
  popped in order on `tool-result'."
  (unless (and (markerp insertion-marker)
               (marker-buffer insertion-marker))
    (error "INSERTION-MARKER must be a live marker"))
  (unless (marker-insertion-type insertion-marker)
         ...
         (let (last-marker)
           (dolist (call calls)
             (let ((m (gptel-chat--stream-open-tool-block
                       insertion-marker call)))
               (setq pending-tool-markers
                     (append pending-tool-markers (list m))
                     last-marker m)))
           (when last-marker
             (funcall set-tool last-marker))))
        ;; Tool-result event: match each result to the next pending
        ;; marker in FIFO order, insert the result and close the
        ;; block.  Once all results have been processed, clear the
        ;; tool-marker override so subsequent streaming goes back
        ;; to the assistant-level marker.
        (`(tool-result . ,results)
