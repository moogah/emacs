...
;; Forward declaration for the heading-collision escape width.  The
;; defcustom is owned by `gptel-chat-mode' (config/gptel/chat/mode.el)
;; — see chat.org for load order.  Production loads `mode.el' before
...
;; input unchanged or a version with a single escape prefix prepended.
;; Two mutually-exclusive collision rules apply, in this order:

;; 1. *Delimiter collision* (existing). The line matches
;;    =gptel-chat--end-delimiter-regexp= (case-insensitive
;;    =^#\+end_\(user\|assistant\|tool\)\b=). The escape is a single
;;    leading =,= so org's delimiter scanner no longer sees the line as
;;    a closer. The parser's send-path =unescape= strips the leading =,=
;;    before the message reaches the model.
;; 2. *Heading collision* (new — design.md §Decision 4 of
;;    =openspec/changes/gptel-chat-heading-scoping=). The line matches
;;    =^\*+ = (one or more =*= at column 0 followed by a space). Such a
;;    line, inside a chat-block body, is parsed by =org-element= as a
;;    heading and destroys the special-block AST. The escape is
;;    =(make-string gptel-chat-content-indentation ?\s)= leading spaces
;;    so the heading regex (anchored at column 0) no longer matches.
;;    The parser's send-path strips any leading whitespace before such
;;    a line, so round-trip is robust against config changes.

;; The two rules are mutually exclusive at the line level: a line that
;; starts with =#+end_= cannot also start with =*=. Order is therefore
;; not load-bearing for correctness — it is chosen for code clarity and
;; for stable behaviour if the regexes are ever loosened.

;; Lines that match neither rule pass through unchanged. In particular
;; =#+end_src=, =#+begin_assistant=, prose beginning with a colon, and
;; indented =*= or =#+end_*= lines (anything not at column 0) are
;; explicitly preserved. Design.md §Decision 4 (chat-mode parent
;; change) covers why we do **not** use =org-escape-code-in-string=:
;; its broader escape clutters assistant content with spurious commas
;; on benign lines.
...
(defconst gptel-chat--heading-collision-regexp
  "^\\*+ "
  "Regexp matching a line that org would parse as a heading.
One or more `*' at column 0 followed by a space.  Inside a chat-block
body, such a line corrupts `org-element-parse-buffer''s view of the
special block (the heading absorbs the block contents into an outline
subtree).  Used by `gptel-chat--sanitize-chunk' to apply the
heading-collision escape.  Case-insensitive matching is irrelevant
here — the pattern has no letter alternatives — but the regexp is
applied under the same `case-fold-search' binding as the delimiter
regexp for consistency.")

(defun gptel-chat--sanitize-chunk (line)
  "Return LINE with a collision escape prepended when applicable.
LINE must be a complete single line with no embedded newline.  Two
mutually-exclusive collision rules apply, checked in this order:

1. Delimiter collision: LINE matches
   `gptel-chat--end-delimiter-regexp' (case-insensitive
   `^#\\+end_\\(user\\|assistant\\|tool\\)\\b').  Returns LINE with
   a single leading `,' prepended so org's special-block scanner no
   longer recognises the closer.  The parser's send-path
   `gptel-chat--unescape-end-delimiters' strips that `,' before the
   message reaches the model.

2. Heading collision: LINE matches `gptel-chat--heading-collision-regexp'
   (`^\\*+ ' — one or more `*' at column 0 followed by a space).
   Returns LINE with `gptel-chat-content-indentation' leading spaces
   prepended so the column-0 heading anchor no longer matches.  The
   parser's send path strips any leading whitespace before such a
   line, so round-trip is robust against config changes (a buffer
   written under one indentation value re-normalizes when re-opened
   under another).  See
   `openspec/changes/gptel-chat-heading-scoping/design.md' §Decision 4.

The two rules are mutually exclusive at the line level: a line
beginning with `#+end_' cannot also begin with `*'.  Lines matching
neither rule — including `#+end_src', `#+begin_assistant',
already-escaped `,#+end_*' or ` * Heading' lines, indented `*'
content, and ordinary prose — pass through unchanged.

`gptel-chat-content-indentation' is normally bound by `gptel-chat-mode'
\(`config/gptel/chat/mode.el') which loads before this module per
`config/gptel/chat/chat.org'.  When that module has not been loaded
\(notably in standalone test contexts that `require' only
`gptel-chat-stream'), the function falls back to 1 — the
defcustom default — so the function is safe to call regardless.
