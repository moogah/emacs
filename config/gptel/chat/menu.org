...
available within the session scope, and the user is an Emacs user.

The prelude is no longer a hard-coded =defconst= here.  It is the
canonical *static* leading fragment of the chat context, sourced from
=config/gptel/presets/sources/emacs-prelude.org= and pre-rendered to a
...
*chat lead* seam =jf/gptel-fragment-chat-prelude-text= to the rendered
text; the composer's =jf/gptel-fragment--default-composition= reads that
seam and places the prelude as the *leading* static reference of the
chat composition (=register/invariant/context-default-composition=:
chat → =[emacs-prelude(static), role, environment(dynamic)]=), so it is
present even when the buffer carries no role.  Empty/nil contributions
are skipped by the composer, so the prelude still leads.

This module only consumes the composer; it neither defines the prelude
text nor re-implements composition.  The seam defaults to =""= until the
prelude source loads (an empty, skipped contribution), so this module
stays load-safe ahead of the presets sources.
