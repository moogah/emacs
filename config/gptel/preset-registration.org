...
4. *Extract scope and mode* -- Strip scope and mode keys, store in side tables
...
subsystem. The legacy =:org-roam-patterns= key is no longer extracted (cycle-3
removed pattern-based validation); presets that still carry it trigger a
=display-warning= but are otherwise registered without the legacy section
flowing into scope-defaults.
...
by the session subsystem when determining file format (org-mode vs markdown-mode).
...
NOTE: This implementation is duplicated in =scope-profiles.org= and
...
Strip the =:mode= key from the plist and store it in
...
coerce, extract scope, extract mode, and register via =gptel-make-preset=.
