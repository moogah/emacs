...
Scope keys (=:paths=, =:org-roam-patterns=, =:shell-commands=, =:scope-profile=) are
stripped from the preset plist and stored here for later use by the scope subsystem.
