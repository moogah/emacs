...
text (Mode 2a). The previous =scope.yml= writer has been removed; callers
that have not yet been rewired (=sessions/commands.org=,
=tools/persistent-agent.org=) will see loud errors until their dedicated
rewire tasks land.
...
NOTE: This implementation is duplicated in =preset-registration.org= and
=scope-shell-tools.org=. The duplication is intentional - early-loaded modules
(preset-registration, scope-profiles) cannot depend on late-loaded modules
(scope-shell-tools requires scope-core and scope-expansion).
