...
(file paths, org-roam patterns, shell commands) that can be associated with
...
* Scope Defaults Variable

Hash table populated by the preset-registration module. Maps preset names to
their scope configuration (either a named profile or inline scope defaults).
