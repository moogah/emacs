...
(file paths, shell commands, bash tools) that can be associated with
...
2. *shell-commands* - Legacy shell command allow/deny lists (deprecated in favor of bash_tools)
3. *bash_tools* - Category-based bash command execution with directory scoping

(Earlier versions of this module also extracted an =org-roam-patterns= section
for org-roam node filtering. Cycle-3 retired pattern-based validation; the
section is no longer consumed and is no longer extracted from preset
config — see =presets/registration.org= for the legacy-key warning.)
...
NOTE: this normalization applies to profile YAML loaded from disk.  The
fragment-era preset registration (=presets/registration.org=) no longer
performs any YAML normalization — preset config is authored in native
Elisp.  The previous shared =scope-yaml.el= boundary module was retired
together with =scope.yml= on-disk persistence; profile YAML is now the
only YAML the scope subsystem touches.
...
(e.g. =:bash_tools=) to kebab-case (=:bash-tools=) and
...
\(writing =scope.yml= as a side effect) has been removed; callers
prepend the returned drawer text to a fresh =session.org='s initial
content themselves.
