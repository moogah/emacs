...
- Scoped file access via =scope.yml= (default: deny-all for security)
...
- Scope-based file access controls (via =scope.yml=)
...
6. Branching - Branch creation and management (requires metadata, filesystem)
7. Commands - User-facing commands (requires branching)
8. Activities Integration - Auto-creation and resume for activities (optional)
