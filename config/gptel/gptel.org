...
To create a persistent gptel session, use =M-x jf/gptel-persistent-session=.
Each session has its own directory with branch-based management,
scoped file access via a =:PROPERTIES:= drawer in =session.org=
(default: deny-all for security), preset model/backend/tool
configuration, and content-addressed activation on open.

(An earlier integration auto-created sessions for each
=activities.el= activity; that integration was removed when the
=workspaces= package replaced =activities-extensions=. A workspace-
scoped equivalent may land later — for now, sessions are created
explicitly via the command above.)
...
Register all preset definitions in =gptel--known-presets=. Presets are authored
as =.org= sources tangling to self-registering =preset.el= artifacts under
=config/gptel/presets/<name>/= (fragment-era pipeline; the former YAML pipeline
is deleted).
...
Sessions are stored in =~/.gptel/sessions/= by default.

See =config/gptel/sessions/commands.org= for complete documentation.

(A previous integration auto-created sessions for each =activities.el=
activity; that integration was removed when the =workspaces= package
replaced =activities-extensions=.)
