...
#### Scenario: Bash resource is a file path
- **WHEN** bash expansion resource is a file path
- **THEN** system delegates to jf/gptel-scope--add-path-to-scope
- **AND** adds the path to the appropriate paths section based on the denied operation
...
**Note**: Function names use `-scope` suffix (not `-preset`). Bash validation type is `bash` (not "command"). Bash commands are expanded via path-based scopes — there is no command-name expansion path, because commands are validated by the file operations they perform.
