...
- **THEN** system adds command to bash_tools.categories section
...
- **THEN** system adds command to bash_tools.categories.read_only.commands list
...
- **THEN** system adds command to bash_tools.categories.safe_write.commands list
- **AND** preserves existing commands in list
- **AND** writes updated structure to scope.yml

#### Scenario: Command already in category list
- **WHEN** user approves a command already in the category list
- **THEN** system does not duplicate the command
...
- **THEN** system creates bash_tools section with default structure
...
- **THEN** system preserves nested structure: bash_tools → categories → read_only/safe_write → commands
...
- **THEN** system reads existing structure
...
- **THEN** system adds (tool-name . resource) to allow-once list
...
#### Scenario: Allow-once cleared after response
- **WHEN** LLM response completes
- **THEN** system clears buffer-local allow-once list
...
- **THEN** system checks command against known read_only and safe_write lists
...
- **THEN** system determines required scope (read vs write) based on command category
