# Grammar Extraction Layer

## Purpose

Provide unconditional grammar-level semantic extraction from parsed bash commands. This layer (Layer 0) operates on shell grammar structures -- compound decomposition and redirections -- without any domain-specific or command-name knowledge.

## Responsibilities

- Recursively decompose all compound bash structures (pipelines, chains, conditionals, loops, subshells, command substitutions, nested commands) into simple commands
- Extract file operations from shell redirections for every simple command encountered
- Dispatch each simple command to the command handler layer for domain-specific extraction
- Merge grammar-level and handler-level operations into a unified result by domain

## Key Invariants

- Compound decomposition runs unconditionally (no predicate gating, no command-name checks)
- Redirection extraction runs for every simple command (not just known file commands)
- Grammar layer produces only `:filesystem` operations (redirections are shell I/O)
- Grammar layer never inspects command names to decide whether to extract
- Simple commands dispatched to handler layer receive accumulated variable/directory context
...
- **Core Parser**: Consumes parsed commands with token inventory and compound structure
- **Command Handler Layer (Layer 1)**: Receives dispatched simple commands for domain-specific extraction
- **Semantic Orchestrator**: Merges Layer 0 and Layer 1 results into final output
- **Coverage System**: Provides claimed token IDs from redirection extraction
