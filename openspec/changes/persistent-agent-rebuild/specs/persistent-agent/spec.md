...
(None at the requirement level. Implementation-level details that no longer apply — the dual-duty callback, the agent-owned auto-save hook, the `denied_paths` parameter — are absorbed into the MODIFIED Requirements above as scenario-level negations.)
