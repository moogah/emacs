...
- One file (`bash-parser-contract-layers-spec.org`) is mixed: Layer 7 tested the operation→scope-section mapping that now lives at `config/gptel/scope/test/core/operation-mapping-spec.el`; Layers 1–6 test the bash-parser ↔ scope-validation contract layers (source-of-truth symmetry, semantic classification, permissive/restrictive properties, real-command corpus, error-shape contract). SURGICAL: delete L7 describe (coverage preserved by `operation-mapping-spec.el`), migrate L1–L6 fixtures.
...
**D5. `bash-parser-contract-layers-spec.org` describe-block surgery.** As implemented, Layer 7 (operation → scope-section mapping) was deleted because its contract is exhaustively covered by the dedicated unit-test file at `config/gptel/scope/test/core/operation-mapping-spec.el`. Layers 1–6 (source-of-truth symmetry, semantic classification, permissive property, restrictive property, real-command corpus, error-shape contract) were retained — they test the bash-parser ↔ scope-validation contract layers, which remain real. Surgery removes ~50 lines from one file (the Layer 7 describe) and rewrites the fixture builders (`contract-layers-spec--permissive-config`, `--restrictive-config`) to use `helpers-spec-make-scope-config`. (Earlier drafts of this design proposed deleting Layers 1–2 instead, which is no longer accurate — those layers were salvageable once the fixture builders were rewritten.)
