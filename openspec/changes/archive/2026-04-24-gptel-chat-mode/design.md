...
| `` `(tool-call . ,calls) `` | **confirmation-path only** — fires once per batch for tools whose `:confirm` predicate is active. Open a nested `#+begin_tool (<name> <plist...>)` block inside the active assistant block, push each result-target marker onto the `pending-tool-markers` FIFO, and set the tool-block marker (Decision 3b) to the LAST opened block so prose arriving before tool-result lands inside it. |
| `` `(tool-result . ,results) `` | For each 3-list: pop the matching marker off `pending-tool-markers` (confirmation path) or synthesize a fresh `#+begin_tool`/`#+end_tool` pair on the fly from the 3-list's TOOL-STRUCT and ARGS (auto-approved path — see note below). Insert the stringified result, which fills in the block body. After all results are handled, clear the tool-block marker so subsequent streaming routes back to the assistant-level marker. |
...
**Tool-call and tool-result are NOT symmetric events.** Upstream `gptel--handle-tool-use` (`gptel-request.el:1684-1752`) only emits `(tool-call . pending-calls)` when at least one tool in the batch requires confirmation — i.e., when `gptel-confirm-tool-calls` is truthy AND the tool's `:confirm` predicate (if any) returns non-nil. Tools that pass the confirmation check execute inline during the `mapc` over `tool-use`, and their results accumulate into `result-alist` without a preceding `tool-call` event. When all tools in the batch finish (auto-approved or confirmed), upstream fires ONE terminal `(tool-result . result-alist)` event carrying every result. The callback must therefore be prepared to render a tool block with no prior notice — it has the TOOL-STRUCT, ARGS, and RESULT on the 3-list itself.

The asymmetry also makes the per-result synthesize path the correct behaviour for a MIXED batch (some confirmed, some auto-approved): confirmed tools' results pop their pre-opened markers off the FIFO, while auto-approved tools' results synthesize a block each. A strict "tool-result requires a preceding tool-call" contract would crash in the common case (every auto-approved tool invocation) and produces a nasty failure mode when the crash is caught by the tool's own `condition-case` wrapper: the wrapped error is fed back through `process-tool-result`, which re-fires the callback with a `tool_exception` payload, which crashes again — the FSM is left wedged in `TOOL` with the assistant block open and no response text, presenting to the user as a "nothing happened" hang.
...
**Corrected from earlier drafts:** three successive drafts of this decision had drift that this final form fixes.
...
2. The first correction over-pivoted and described each list element as a plist carrying `:name` / `:args` / `:result`. That was also wrong (task `stream-callback-tool-element-shape-and-tests`, orch-review-1776774164 Finding #1): upstream emits 3-lists, not keyword-tagged plists. Reading the real shape with `plist-get` yielded nil for every slot and rendered every tool header as `#+begin_tool ( :args nil)` with empty results. Destructure via `pcase` triples, not `plist-get`.
3. The second correction treated `tool-call` and `tool-result` as a matched pair — every `tool-result` was required to have a preceding `tool-call` that had pushed a marker onto `pending-tool-markers`, and a result with no pending marker signalled a loud "orphan tool-result" error. This misread upstream: `tool-call` fires only on the confirmation path, while auto-approved tools surface only via the terminal `tool-result` event. The callback now synthesizes blocks on demand in the tool-result arm (see the table row above), and the `tool-result` triple is destructured for TOOL-SPEC / ARGS — not just RESULT — so the synthesize path has everything it needs.
