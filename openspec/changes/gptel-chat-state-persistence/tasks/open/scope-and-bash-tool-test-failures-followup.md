...
3. Triage by cluster. Each cluster (parallel-callback, bash-tool integration, add-to-scope, timeout messages, bash-parser corpus case) is probably an independent fix. Decide whether to split this task per-cluster once root causes are known.
4. Fix and add regressions. Prefer fixing the production code; only adjust tests if the spec was wrong (justify in commit). For "transient collision" / "session stuck" failures, characterize the bug first (a follow-up bead may already exist for this — search drawer-corruption notes and parallel-callback specs).
5. Re-run `./bin/run-tests.sh` and confirm 0 unexpected failures + 0 aborted + 0 unknown.

## Design rationale

These failures pre-date the gptel-chat-state-persistence change and are out-of-scope for it (the change touched session-creation / metadata-removal code paths, not scope/bash-tool dispatching). The regression sweep surfaced them because the task body asked for a green full-suite run as the verification gate. Capturing them here preserves the audit trail (`discovered-from:` the sweep task) without conflating two unrelated concerns.

The cluster naming above reflects how Buttercup grouped them in the failure listing on 2026-04-25; once bisected, splitting into per-cluster tasks is appropriate if root causes diverge.
...
- Surfaced by: `regression-sweep-and-manual-smoke` (this change's verification task) on 2026-04-25.
