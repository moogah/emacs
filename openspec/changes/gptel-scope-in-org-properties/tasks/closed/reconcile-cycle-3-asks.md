...
- Pre-existing failures (78 of 490 buttercup specs) are unchanged by this task. The two timeout-related failures in the validation directory (`run_bash_command: Timeout and resource limits …`) and the cluster of bash-parser→scope integration / `no_scope_config` callback failures were present on the cycle-3 baseline and remain so. They are out of scope for cycle-4 reconciliation work.
- The directory-scoped `test-report.txt` artefact (`config/gptel/scope/test/validation/test-report.txt`) generated by `./bin/run-tests.sh --report -d <dir>` is left untracked — only the suite-level `config/gptel/scope/test-report.txt` is git-tracked, matching prior commits' precedent.
- The `register/shape/scope-config-plist` validator field gained a new `cl-every #'keywordp` clause for `:cloud.allowed-providers`; this is a documentation contract, not executable code. No callers were touched. (Kept inside the existing register entry; not promoted to a separate invariant.)
- The historical `:match-pattern` decision_note retains the pointer to the cycle-2 `harden-add-to-scope-action-handler` task name even though that task is now closed; the provenance trail is more useful than a dangling-reference clean-up at this point.

## Discoveries

- discovery_id: disc-reconcile-cycle-3-asks-1
  class: spec-signal
  description: |
    The cycle-2 ask-10B "smart redirect" decision in
    `register/vocabulary/operation-to-drawer-key` was structurally
    blocked by the validator's first-denial-wins behaviour
    (`scope-validation.el:412`'s `(catch 'error-found ...)`),
    yet the register entry advertised the decision as confirmed.
    The cycle-3 implementation discovered the gap; cycle-4
    Ask 1 reconciled the disposition into "refuse with guidance"
    and externalized the parser/validator boundary work to
    `.tasks/fix-match-pattern-parser-validator-boundary.md`.
    This task encodes that disposition into the register; the
    deeper finding (validator should reify clusters before
    handing them to the action handler) remains open in `.tasks/`.
  affected_register_entry: register/vocabulary/operation-to-drawer-key
  recommendation: |
    When the externalized `.tasks/fix-match-pattern-parser-validator-boundary.md`
    work is picked up, revisit Stage 2 of
    `register/boundary/scope-expansion-action-handler` and the
    `:match-pattern` member of
    `register/vocabulary/operation-to-drawer-key`: the redirect
    disposition may become implementable, at which point Stage 2
    can be renamed back (or split into a "refuse" → "redirect"
    mode toggle) and the vocabulary `redirect_target` field can
    be reintroduced. The current Option-B contract is therefore
    cycle-3-as-shipped, not architecturally final.

- discovery_id: disc-reconcile-cycle-3-asks-2
  class: shape-fragmentation
  description: |
    Pre-cycle-4, `:cloud.allowed-providers` was a list of strings
    in the loader's output but a list of keywords at the
    validator's comparison site (`(member provider allowed-providers)`
    where `provider` is the cloud-auth ops detector's keyword
    output). This silent type mismatch meant the
    `cloud_provider_denied` filter never matched (every keyword
    provider failed the membership check, so denial was ambient
    rather than configurable). The cycle-3 reviewer F2 finding
    surfaced this; cycle-4 Ask 2 closes it at the loader as the
    parse-boundary normalisation point.
  affected_register_entry: register/shape/scope-config-plist
  recommendation: |
    Two follow-up surfaces worth a future sweep: (1) any other
    drawer shape where the validator works in a different type
    than `org-entry-get-multivalued-property` returns — at minimum
    audit the path-list keys for path normalisation (e.g. trailing
    slashes, `~/` expansion); (2) a structural test that
    enumerates every key the validator reads from the plist and
    asserts the loader's output type matches each consumer's
    expected type, to keep this class of mismatch from
    re-emerging silently.

- discovery_id: disc-reconcile-cycle-3-asks-3
  class: duplication
  description: |
    The fixture helper `jf/gptel-test--render-drawer` previously
    emitted multi-value drawer keys as `:KEY: v1\n:KEY+: v2\n`
    while the production writer
    `jf/gptel-scope--write-pattern-to-drawer` emits the
    space-separated single-line form via
    `org-entry-put-multivalued-property`. The two encodings are
    semantically equivalent under
    `org-entry-get-multivalued-property`, but tests exercising
    fixture text were not byte-equivalent to real session.org
    drawer state. Cycle-4 Ask 3 unifies the encoding by rewriting
    the fixture helper to match the writer; the helpers-spec self-
    test now byte-compares the two encodings to keep them aligned.
  affected_register_entry: register/boundary/scope-pattern-writer
  recommendation: |
    Consider promoting the fixture-helper-vs-writer byte-equivalence
    self-test to an explicit invariant (e.g.
    `register/invariant/drawer-fixture-text-matches-writer-output`).
    The current helpers-spec coverage pins it for one input shape
    (a multi-value READ key with three values); a future invariant
    could property-check across the closed set of drawer keys.
